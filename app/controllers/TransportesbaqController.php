 ...
 */
class TransportesbaqController extends BaseController{
  ...
  public function getIndex(){
    ...
    if (!Sentry::check()){
        ...
        if($id_rol==1 or $id_rol==2 or $id_rol==8){
            ...
            $servicios = $query->whereNull('control_facturacion')->orderBy('hora_servicio')->get();
        }
        ...
        return View::make('servicios.transportesbaq', [
      ...
      if (!Sentry::check()){
          ...
          if (Request::ajax()){
              ...
              $consulta = "select servicios.id, servicios.fecha_orden, servicios.fecha_servicio, servicios.solicitado_por, servicios.programado_app, ".
                  ...
                  "where servicios.fecha_servicio between '".$fecha_inicial."' AND '".$fecha_final."' and servicios.localidad is null";
      ...
      if (!Sentry::check()){
          ...
          if (Request::ajax()){
              ...
              $consulta = "select servicios.id, servicios.fecha_orden, servicios.fecha_servicio, servicios.solicitado_por, servicios.programado_app, ".
                  ...
                  "left join subcentrosdecosto on servicios.subcentrodecosto_id = subcentrosdecosto.id ".
                  "left join users on servicios.creado_por = users.id ".
                  "where servicios.fecha_servicio between '".$fecha_inicial."' AND '".$fecha_final."' and servicios.localidad is null and servicios.centrodecosto_id =  329";
                  ...
                  if($ordenes_facturacion!=null or $liquidacion!=null){
                      ...
                      if ($servicios->save()) {
                          ...
                          if ($conductor!=$servicios->conductor_id) {
                              ...
                              // 1. notificacion de cancelacion (CONDUCTOR)
                          ...
                          if($servicios->app_user_id!=null && $servicios->app_user_id!=0){ //Si es servicio de la app
                            ...
                            //si la fecha y la hora actuales son menores a la fecha y hora del servicio
                            if( ($actualDate<$servicios->fecha_servicio) || ($actualDate=$servicios->fecha_servicio && $actualTime<=$servicios->hora_servicio) ){
                              //si hubo cambios en el conductor, fecha u hora.
                          ...
                          if ($cambios!='') {
                              ...
                              //si la fecha y la hora actuales son menores a la fecha y hora del servicio
                              if($actualFecha<=$servicios->fecha_servicio){
                                ...
                                if ($cambiosConductor===false) {
                                  ...
                                  $responseNotificacion = Servicio::NotificacionesUpdate($cambiosNotificaciones, $servicios->conductor_id);
                                }
  ...
  public function postMostrarconductoresyvehiculos(){
      ...
      if(Request::ajax()){
          ...
          $conductores = "SELECT id, nombre_completo, celular, bloqueado, bloqueado_total FROM conductores WHERE proveedores_id = ".Input::get('proveedores_id')." ORDER BY nombre_completo";
          $conductores = DB::select($conductores);
          
          $vehiculos = "SELECT id, placa, clase, modelo, color, marca, ano, capacidad, bloqueado, bloqueado_total, archivado FROM vehiculos WHERE proveedores_id = ".Input::get('proveedores_id')." ORDER BY placa";
              ...
              if ($validador->fails()){
                  ...
                  for($i=0; $i<$cantidadArrays; $i++){
                      ...
                      if($ordenes_facturacion!=null or $liquidacion!=null){
                          ...
                          $servicio->hora_programado_app = date('Y-m-d H:i:s');
                          if ($kilometrajeArray[$i]==='SI') {
                            $servicio->estado_km = 1;
                          }
                          ...
                          if($pago_directoArray[$i]==1){

                            if($metodo_pagoArray[$i]==2){
                              $pagoWompi = new PagoServicio;
                              $pagoWompi->servicio_id = $servicio->id;
                              $pagoWompi->link = 1;
                              $pagoWompi->save();
                            }
                          }
  ...
  public function getServiciosporeliminar(){
      ...
      if (!Sentry::check()){
          ...
          $servicios = DB::table('servicios')
              ...
              ->where('servicios.fecha_servicio', '>', '20220701')
  ...
  public function postEnviarnotificacion(){
    ...
    if($fechaActual<=$servicio->fecha_servicio){
          ...
          if($ordenes_facturacion!=null){
            ...
            if ($ruta!=Input::get('ruta_id')) {
              ...
              if ($ruta==null) {
                ...
                $rutas2 = Traslado::where('id', Input::get('ruta_id'))->first();
                $cambios = $cambios.'Se asigno el nombre de ruta <b>'.$rutas2->tarifa_nombre.'</b><br>';

              }else {

                //Anterior
                $rutas1 = Traslado::where('id', $ruta)->first();
                ...
                if (!count($rutas1)) {
                  ...
                  if( isset($rutas1) and isset($rutas2)){
  ...
  public function postEnviarmailservicios(){
    ...
    if ($validator->fails()) {
      ...
      function($message) use ($correo_electronico, $asunto, $cc_correo_electronico){
          ...
          $message->from('transportebarranquilla@aotour.com.co', 'TRANSPORTES - AOTOUR');
  ...
  public function getServiciosporprogramar(){
    ...
    if (Sentry::check()){
        ...
        $permisos = DB::table('roles')->where('id',$id_rol)->pluck('permisos');
        $permisos = json_decode($permisos);
        $ver = $permisos->barranquilla->ejecutivosbq->ver;
    ...
    if (!Sentry::check()){
      ...
      $proveedores = Proveedor::Valido()->afiliadosinternos()->whereIn('localidad',['barranquilla','provisional'])->orderBy('razonsocial')->get();
      ...
      ->where('servicios_autonet.localidad','barranquilla')
      ->orderBy('date')
      ->orderBy('time')->get();

      return View::make('servicios.servicios_ejecutivos.por_programar')
      ->with('centrosdecosto',$centrosdecosto)
      ->with('proveedores',$proveedores)
      ->with('departamentos',$departamentos)
    ...
    if ($validador->fails()){
      ...
      $consulta = "SELECT * FROM liquidacion_servicios WHERE '".Input::get('fecha_servicio')."' BETWEEN fecha_inicial AND fecha_final and centrodecosto_id = '".Input::get('centrodecosto')."' and subcentrodecosto_id = '".Input::get('subcentrodecosto')."' and ciudad = '".Input::get('ciudad')."' and anulado is null and nomostrar is null";
        ...
        if($liquidacion!=null or $ordenes_facturacion!=null){
            ...
            $servicio = new Servicio();
            $servicio->fecha_orden = Input::get('fecha_solicitud');
            $servicio->centrodecosto_id = Input::get('centrodecosto');
            $servicio->subcentrodecosto_id = Input::get('subcentrodecosto');
            $servicio->pasajeros = $pasajeros_todos;
            ...
            if ($servicio->save()) {
              ...
              $name_first = explode(" ", $consulta_name);
                ...
                //ENVIO DE MENSAJE CONDUCTOR
                //$nov = 'rutasbogota@aotour.com.co';
                if($servicio->proveedor_id!=45 and $servicio->proveedor_id!=336 and $numberok!=null){
                  $post['to'] = array('57'.$numberok);
                  $post['text'] = "Sr(a) ".$name_first[0].", se le ha programado un servicio ejecutivo para el dia ".$servicio->fecha_servicio.", a las ".$servicio->hora_servicio.". Por favor, revise su APP!";
                  $post['from'] = "msg";
                  $user ="Aotour";
                  $password = 'Tour20+';
                  $ch = curl_init();
              ...
              if($email_cliente==1){
                ...
                $sw = 0;
                  ...
                  $email = Input::get('email_solicitante');
                  $sw++;
                  //QUERY EMPRESA
                  //$empresa = DB::table('centrosdecosto')
                  //->where('id',$servicio_email->id)->pluck('razonsocial');
                  ...
                  if(Input::get('centrodecosto')==343){
                    ...
                    //$emailcc = ['lgiraldo@coninsa.co', 'llopez@coninsa.co', 'mmoreno@coninsa.co', 'amosorio@coninsa.co', 'dasanchez@coninsa.co', 'sramirez@coninsa.co', 'anaruiz@coninsa.co', 'lfarroyave@coninsa.co', 'asistentecomercial@coninsa.co', 'lctorom@coninsa.co', 'ttobon@coninsa.co', 'tifigueroa@coninsa.co', 'gcardona@coninsa.co', 'dcreyes@coninsa.co'];
                    //$emailcc = ['sistemas@aotour.com.co', 'asesoriaqr@aotour.com.co'];
                    //$email = $email_pasajeros;//'sdgonzalezmendoza@gmail.com';

                    Mail::send('servicios.servicios_ejecutivos.cliente.email_programado', $data, function($message) use ($email){
                      $message->from('no-reply@aotour.com.co', 'AOTOUR');
                      $message->to($email)->subject('Programación de Servicio');
  ...
  public function getPoriniciar(){
    ...
    if (!Sentry::check()){
        ...
        return View::make('servicios.por_iniciarbq', [
          'servicios' => $servicios,
          'permisos' => $permisos,
          'o' => $o,
      ...
      if (!Sentry::check()){
        ...
        if (Request::ajax()){
          ...
          $consulta = "select servicios.id, servicios.fecha_servicio, ".
            ...
            "left join centrosdecosto on servicios.centrodecosto_id = centrosdecosto.id ".
            "left join reconfirmacion on servicios.id = reconfirmacion.id_servicio ".
            "where servicios.fecha_servicio between '".$fechaActual."' AND '".$fechaActual."' and servicios.localidad IS NULL AND servicios.estado_servicio_app IS NULL and servicios.hora_servicio > '".$horaActual."'";
  ...
  public function postBuscarordeness(){
      ...
      if (!Sentry::check()){
          ...
          if (Request::ajax()){
              ...
              $consulta2 = "SELECT servicios.id, servicios.pasajeros, servicios.app_user_id, servicios.fecha_servicio, servicios.hora_servicio, servicios.detalle_recorrido, servicios.conductor_id, servicios.centrodecosto_id, servicios.aceptado_app, servicios.estado_servicio_app, servicios.pendiente_autori_eliminacion, servicios.ruta, servicios.localidad, servicios.recoger_en, servicios.dejar_en, servicios.estado_km, centrosdecosto.razonsocial as razonsocial, conductores.nombre_completo, conductores.celular FROM `servicios` left join centrosdecosto on servicios.centrodecosto_id = centrosdecosto.id left join sin_inicio on sin_inicio.servicio_id = servicios.id left join conductores on conductores.id = servicios.conductor_id WHERE servicios.fecha_servicio between '".$fechaMinima."' and '".$fechaMaxima."' AND servicios.hora_servicio >= '".$horaMinima."' AND servicios.hora_servicio <= '".$horaMaxima."' AND servicios.fecha_servicio between '".$fechaMinima."' and '".$fechaMaxima."' AND (servicios.estado_servicio_app IS NULL) and servicios.pendiente_autori_eliminacion is null AND servicios.localidad is null and (sin_inicio.time <= '".$horaAlerta."' OR sin_inicio.time is NULL) order by servicios.fecha_servicio asc, servicios.hora_servicio asc";
              ...
              if($cantidad==0){
                ...
                $consulta2 = "SELECT servicios.id, servicios.pasajeros, servicios.app_user_id, servicios.fecha_servicio, servicios.hora_servicio, servicios.detalle_recorrido, servicios.conductor_id, servicios.centrodecosto_id, servicios.aceptado_app, servicios.estado_servicio_app, servicios.pendiente_autori_eliminacion, servicios.ruta, servicios.localidad, servicios.recoger_en, servicios.dejar_en, servicios.estado_km, centrosdecosto.razonsocial as razonsocial, conductores.nombre_completo, conductores.celular FROM `servicios` left join centrosdecosto on servicios.centrodecosto_id = centrosdecosto.id left join sin_inicio on sin_inicio.servicio_id = servicios.id left join conductores on conductores.id = servicios.conductor_id WHERE servicios.fecha_servicio between '".$fechaMinima."' and '".$fechaMaxima."' AND servicios.hora_servicio >= '".$horaMinima."' AND servicios.hora_servicio <= '".$horaMaxima."' AND (servicios.estado_servicio_app IS NULL) and servicios.pendiente_autori_eliminacion is null AND servicios.localidad is null and (sin_inicio.time <= '".$horaAlerta."' OR sin_inicio.time is NULL) order by servicios.fecha_servicio asc, servicios.hora_servicio asc";
  ...
  //START SERVICIOS POR INCIAR
  public function postReconfirmarconductorinicio(){

    $id = Input::get('id');

    $consulta = DB::table('sin_inicio')->where('servicio_id',$id)->first();

    $servicio = DB::table('servicios')->where('id',$id)->first();

    if($consulta!=null){

      $fechaNueva = date('Y-m-d H:i:s',strtotime('+10 minute',strtotime(date('Y-m-d H:i:s'))));

      $actualizar = DB::table('sin_inicio')
      ->where('servicio_id',$id)
      ->update([
        'time' => $fechaNueva
      ]);
      ...
      $not = Servicio::Reconfirmarinicio($servicio->conductor_id, $servicio->fecha_servicio, $servicio->hora_servicio, $servicio->recoger_en, $servicio->dejar_en, $message, $servicio->id);

    }else{

      $fechaNueva = date('Y-m-d H:i:s',strtotime('+10 minute',strtotime(date('Y-m-d H:i:s'))));

      $reconfirmador = new Sin();
      $reconfirmador->time = $fechaNueva;
      $reconfirmador->user = Sentry::getUser()->id;
      $reconfirmador->servicio_id = $id;
      $reconfirmador->save();
  ...
  public function getOrdenes(){
    ...
    if (!Sentry::check()){
        ...
        return View::make('servicios.ordenesbq', [
          'servicios' => $servicios,
          'permisos' => $permisos,
          'o' => $o,
          'fechaActual' => $fechaActual,
      ...
      if (!Sentry::check()){
          ...
          if (Request::ajax()){
              ...
              if( $hoy==null ){
                ...
                Image::make($qr_code)->save('biblioteca_imagenes/control_ingreso/'.$data.'.png'); //Creación del QR P2

                $ingresoqr = new IngresoQR;
                $ingresoqr->codigo = $data;
                $ingresoqr->fecha = $fecha;
      ...
      if (!Sentry::check()){
        ...
        if (Request::ajax()){
          ...
          $consulta = "select servicios.id, servicios.fecha_servicio, ".
            ...
            "left join centrosdecosto on servicios.centrodecosto_id = centrosdecosto.id ".
            "left join reconfirmacion on servicios.id = reconfirmacion.id_servicio ".
            "where servicios.fecha_servicio between '".$fechaActual."' AND '".$fechaActual."' and servicios.localidad IS NULL AND servicios.estado_servicio_app = 2";
      ...
      if (!Sentry::check()){
        ...
        if (Request::ajax()){
          ...
          $consulta = "select servicios.id, servicios.fecha_servicio, ".
            ...
            "left join centrosdecosto on servicios.centrodecosto_id = centrosdecosto.id ".
            "left join reconfirmacion on servicios.id = reconfirmacion.id_servicio ".
            "where servicios.fecha_servicio between '".$fechaActual."' AND '".$fechaActual."' and servicios.localidad IS NULL AND servicios.estado_servicio_app = 1";
  ...
  public function postExpediente(){
    ...
    ->select('servicios.*', 'facturacion.liquidacion_id')
    ...
    if($consultaExpediente!=null){ //Si el expediente ya fue asignado antes y no está facturado el servicio, se añade el servicio al mismo expediente
      ...
      $name = DB::table('subcentrosdecosto')->where('id',$subcentro)->pluck('nombresubcentro');
      ...
      $consulta = "SELECT id, nombresubcentro, centrosdecosto_id FROM subcentrosdecosto WHERE centrosdecosto_id = 329 AND nombresubcentro LIKE '%BAQ%' AND nombresubcentro NOT LIKE '%CARBONES%' AND id != 1051";
      ...
      $name = DB::table('subcentrosdecosto')->where('id',$data)->pluck('nombresubcentro');
  ...
  public function getExpedientes(){
    ...
    if (!Sentry::check()){
        ...
        $query = DB::table('servicios')
            ...
            ->select('servicios.*',
                 ...
                 'vehiculos.placa','vehiculos.clase','vehiculos.marca','vehiculos.modelo',
                 'servicios_edicion_pivote.id as id_pivote', 'reportes_pivote.id as id_reporte', 'servicios_autorizados_pdf.documento_pdf1', 'servicios_autorizados_pdf.documento_pdf2')
            ->where('servicios.centrodecosto_id',329)
            ->where('servicios.anulado',0)
            ->whereNull('servicios.localidad')
            ->whereRaw('(servicios.cancelado=0 or servicios.cancelado is null)')
            ->whereNull('servicios.pendiente_autori_eliminacion')
            ...
            return View::make('servicios.expedientes', [
              'servicios' => $servicios,
              'proveedores' => $proveedores,
              'permisos' => $permisos,
              'usuarios' => $usuarios,
    ...
    public function postBuscarservicio() {

      $id = Input::get('id');

      $servicio = DB::table('servicios')
      ->leftJoin('subcentrosdecosto', 'subcentrosdecosto.id', '=', 'servicios.subcentrodecosto_id')
      ->select('servicios.id','servicios.fecha_servicio', 'servicios.expediente', 'subcentrosdecosto.nombresubcentro')
      ->where('servicios.id',$id)
      ->first();
      ...
      $fechaFinal = strtotime ('+20 day', strtotime($fecha));
      $fechaFinal = date('Y-m-d' , $fechaFinal);

      $otrosServicios = DB::table('servicios')
      ->leftJoin('subcentrosdecosto', 'subcentrosdecosto.id', '=', 'servicios.subcentrodecosto_id')
      ->select('servicios.id','servicios.fecha_servicio', 'servicios.expediente', 'subcentrosdecosto.nombresubcentro')
      ->where('centrodecosto_id',329)
      ->whereBetween('fecha_servicio',[$fechaInicial,$fechaFinal])
      ->whereNull('pendiente_autori_eliminacion')
      ->where('expediente',$servicio->expediente)
      ->where('servicios.id','<>',$id)
      ->get();
    ...
    public function postActualizar() {
      ...
      $servicio_id = Input::get('servicio_id');
      $expediente = Input::get('expediente');
      $subcentro = Input::get('subcentro');

      $update = DB::table('servicios')
      ->where('id',$servicio_id)
      ->update([
        'expediente' => $expediente,
        'subcentrodecosto_id' => $subcentro
      ]);
