...
class TransportesrutasqrController extends BaseController{
    ...
    public function getIndex(){
        ...
        if (!Sentry::check()){
            ...
            return View::make('servicios.rutasqr')
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()){
                ...
                $consulta = "select servicios.id, servicios.id as servicioid, servicios.fecha_orden, servicios.fecha_servicio, servicios.solicitado_por, ".
                    ...
                    "where servicios.fecha_servicio between '".$fecha_inicial."' AND '".$fecha_final."' and servicios.ruta is not null and users.localidad in (1,2) ";
    ...
    public function postMostrarproveedores(){
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()) {
                ...
                $proveedores = DB::select('select * from proveedores where inactivo is null and localidad = "barranquilla" order by razonsocial');
        ...
        if (!Sentry::check()){
            ...
            Excel::selectSheetsByIndex(0)->load(Input::file('excel'), function($reader) use ($nombres_ruta){
                ...
                foreach($result as $value){
                    ...
                    /*$array = explode(' ',$apellidos_excel);  // convierte en array separa por espacios;
                    $salida ='';
                    ...
                    if(is_string($value[1])){
                        ...
                        $dataExcel = [
                           'nombres' => strtoupper(trim($apellidos_excel)), //CEDULA
                           'apellidos' => strtoupper(trim($nombre_excel)), //NOMBRES
                           'cedula' => trim($val_cel), //TELEFONO
        ...
        if (!Sentry::check()){
            ...
            for ($i=0; $i < count($rutaIdArray); $i++) {
                ...
                if($liquidacion!=null or $ordenes_facturacion!=null){
                  ...
                  //$servicio->conductor_id = $conductorIdArray[$i];
                  //$servicio->vehiculo_id = $vehiculoplacaArray[$i];
            ...
            if($global==1){
              ...
              for ($i=0; $i < count($rutaIdArray); $i++) {
                  ...
                  if($liquidacion!=null or $ordenes_facturacion!=null){
                      ...
                      $rutaDoble = DB::table('servicios')
                      ->where('fecha_servicio', Input::get('fecha_servicio'))
                      ->where('conductor_id',$conductorIdArray[$i])
                      ->where('hora_servicio',$horaArray[$i])
                      ->whereNotNull('ruta')
                      ...
                      if($rutaDoble!=null){
                        ...
                        foreach ($rutaDoble as $key) {

                          if( ($key->recoger_en=='SUTHERLAND' and $recoger_enArray[$i]=='SUTHERLAND') or ($key->dejar_en=='SUTHERLAND' and $dejar_enArray[$i]=='SUTHERLAND') ){
                            $controlla = 1;
                            $idss = $key->id;
                          }
                      ...
                      //if(Sentry::getUser()->id_rol != 8 and $controlla==1){
                      if(1>2){
                        ...
                        $paxs = json_decode($rutaDoble->pasajeros_ruta);
                        foreach ($pasajeros[$i] as $item){
                          ...
                          $data = $code.$fecha.$hora.$rutaDoble->id.$rutaDoble->conductor_id; //Valor de Código QR - CONCATENACIÓN
                          ...
                          $usuarioRuta->servicio_id = $rutaDoble->id;
                          $usuarioRuta->fullname = trim($item->nombres);
                          $usuarioRuta->address = trim($address);
                          $programa = strtoupper(trim($item->sub_area));
                          $programa = str_replace('AMP;', '', $programa);
                          $programa = str_replace('amp;', '', $programa);
                          $usuarioRuta->employ = $programa; //Si aplica
                          $usuarioRuta->neighborhood = trim($neighborhood);
                          ...
                          //if(2>1){ //Input::get('mensajesqr')
                          $actualDate = date('Y-m-d');
                          $actualTime = date('H:i');

                          $horaMaxima = date('H:i',strtotime('+30 minute',strtotime($horaArray[$i])));

                              //if(Input::get('notificacion')==='1'){

                                //si la fecha y la hora actuales son menores a la fecha y hora del servicio
                          if($actualDate<=Input::get('fecha_servicio')){
                              ...
                              if($actualTime<=$horaMaxima){
                                  ...
                                  ->where('id',$rutaDoble->conductor_id)
                                      ...
                                      $id_registration = $user->idregistrationdevice;
                                        ...
                                        if ($id_registration!=null and $id_registration!='') {
                                          ...
                                          if($user->idioma==='en'){
                                            ...
                                            $message = 'Your service has been scheduled for '.$fecha.' at '.$hora.'. The driver '.$conductor.' will be in charge of making the transfer. The vehicle is a RENAULT TWINGO with license plate GPQ031. Thanks for trusting us.';
                                          }else{
                                            $title = 'Asignación de Ruta';
                                            $message = 'Su ruta ha sido programada para '.$dia.' a las '.$hora.'. El conductor '.$nameConductor.' será el encargado de realizar el servicio de transporte en el vehículo de placa '.$placa.'. ¡Le deseamos un feliz viaje!';
                                          }
                                          ...
                                          if($user->idioma==='en'){
                                            ...
                                            $title = 'Tu QR está disponible...';
                                            $message = 'Presiona aquí para ver los detalles de tu ruta y visualizar tu QR.';
                                          }
                        ...
                        ->update([
                          'pasajeros_ruta' => json_encode($paxs),
                          'estado_km' => 1
                        ]);

                        //$rutaDoble->pasajeros_ruta = json_encode($paxs);
                        //$rutaDoble->save();
                        ...
                        $proveedor = DB::table('conductores')
                        ->where('id',$conductorIdArray[$i])
                        ->pluck('proveedores_id');

                        $servicio->estado_email = 0;
                        //$servicio->ruta_id = Input::get('tipo_traslado');
                        if($recoger_enArray[$i]=='Nte-Centro-Riomar' or $recoger_enArray[$i]=='METROPOLITANO' or $recoger_enArray[$i]=='Sur occ y Sur Orien'){

                          $reco = 'BAQ';

                          if($recoger_enArray[$i]=='Nte-Centro-Riomar'){
                              $servicio->ruta_id = 34;
                          }else if($recoger_enArray[$i]=='METROPOLITANO'){
                              $servicio->ruta_id = 35;
                          }else if($recoger_enArray[$i]=='Sur occ y Sur Orien'){
                              $servicio->ruta_id = 33;
                          }else{
                              $servicio->ruta_id = 61;
                          }
                          ...
                          if($recoger_enArray[$i]=='GALAPA'){
                              $servicio->ruta_id = 38;
                          }else if($recoger_enArray[$i]=='SOL'){
                              $servicio->ruta_id = 37;
                          }else if($recoger_enArray[$i]=='MALAMBO'){
                              $servicio->ruta_id = 36;
                          }else if($recoger_enArray[$i]=='PUERTO'){
                              $servicio->ruta_id = 39;
                          }else{
                              $servicio->ruta_id = 61;
                          }
                        ...
                        $servicio->recoger_en = $reco; //Nuevo
                        if(Input::get('centrodecosto')==19 and $recoger_enArray[$i]=='SUTHERLAND'){
                          $servicio->desde = '[{"lat":"11.011385","lng":"-74.799179"}]'; //Nuevo
                        }else if( (Input::get('centrodecosto')==257 and $recoger_enArray[$i]=='PIMSA') || (Input::get('centrodecosto')==474 and $recoger_enArray[$i]=='EMERGIA') ){
                          $servicio->desde = '[{"lat":"10.835939824021477","lng":"-74.77095301524687"}]'; //Nuevo
                        }else if( (Input::get('centrodecosto')==462 and $recoger_enArray[$i]=='SMART OFFICE') ){
                          $servicio->desde = '[{"lat":"11.004217894933252","lng":"-74.81313508871034"}]'; //Nuevo
                        }else{
                          $servicio->desde = $recoger_enArray[$i]; //Nuevo
                        }
                        //$servicio->desde = $recoger_enArray[$i]; //Nuevo

                        if($dejar_enArray[$i]=='Nte-Centro-Riomar' or $dejar_enArray[$i]=='METROPOLITANO' or $dejar_enArray[$i]=='Sur occ y Sur Orien'){

                          $deja = 'BAQ';

                          if($dejar_enArray[$i]!='SUTHERLAND'){
                              if($dejar_enArray[$i]=='Nte-Centro-Riomar'){
                                  $servicio->ruta_id = 34;
                              }else if($dejar_enArray[$i]=='METROPOLITANO'){
                                  $servicio->ruta_id = 35;
                              }else if($dejar_enArray[$i]=='Sur occ y Sur Orien'){
                                  $servicio->ruta_id = 33;
                              }else{
                                  $servicio->ruta_id = 61;
                              }
                          ...
                          if($dejar_enArray[$i]!='SUTHERLAND' and $dejar_enArray[$i]!='PIMSA' and $dejar_enArray[$i]!='EMERGIA' and $dejar_enArray[$i]!='SMART OFFICE'){
                              if($dejar_enArray[$i]=='GALAPA'){
                                  $servicio->ruta_id = 38;
                              }else if($dejar_enArray[$i]=='SOL'){
                                  $servicio->ruta_id = 37;
                              }else if($dejar_enArray[$i]=='MALAMBO'){
                                  $servicio->ruta_id = 36;
                              }else if($dejar_enArray[$i]=='PUERTO'){
                                  $servicio->ruta_id = 39;
                              }else{
                                  $servicio->ruta_id = 61;
                              }
                        ...
                        $servicio->dejar_en = $deja; //Nuevo
                        if(Input::get('centrodecosto')==19 and $dejar_enArray[$i]=='SUTHERLAND'){
                          $servicio->hasta = '[{"lat":"11.011385","lng":"-74.799179"}]'; //Nuevo
                        }else if( (Input::get('centrodecosto')==257 and $dejar_enArray[$i]=='PIMSA') || (Input::get('centrodecosto')==474 and $dejar_enArray[$i]=='EMERGIA') ){
                          $servicio->hasta = '[{"lat":"10.835939824021477","lng":"-74.77095301524687"}]'; //Nuevo
                        }else if( (Input::get('centrodecosto')==462 and $dejar_enArray[$i]=='SMART OFFICE') ){
                          $servicio->hasta = '[{"lat":"11.004217894933252","lng":"-74.81313508871034"}]'; //Nuevo
                        }else{
                        ...
                        $ruta_query = explode(" ", $detalleArray[$i]);
                        $rnid = DB::table('nombre_ruta')->where('nombre',$ruta_query[0])->pluck('id');
                        $servicio->ruta_nombre_id = $rnid;
                        $servicio->save();

                        $number = rand(10000000, 99999999);
                        //ENVÍO DE NOTIFICACIÓN AL CONDUCTOR
