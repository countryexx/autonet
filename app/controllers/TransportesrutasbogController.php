...
class TransportesrutasbogController extends BaseController{
    ...
    public function getIndex(){
        ...
        if (!Sentry::check()){
            ...
            $subcentros = DB::table('subcentrosdecosto')/*->whereIn('id', [825, 846, 847, 848])*/->orderBy('nombresubcentro')->get();
            $departamentos = DB::table('departamento')->get();
            $ciudades = DB::table('ciudad')->orderBy('ciudad')->get();
            $rutas = DB::table('rutas')->get();
            $usuarios = DB::table('users')->where('permissions',10)->where('activated',1)->get();

			      $cotizaciones = DB::table('cotizaciones')->where('estado',1)->orderBy('created_at', 'desc')->take(10)->get();

            return View::make('servicios.rutasbogota')
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()){
                ...
                $tipo_usuario = Sentry::getUser()->id_tipo_usuario;
                    ...
                    $consulta = "select servicios.id, servicios.ruta_qr, servicios.id as servicioid, servicios.fecha_orden, servicios.fecha_servicio, servicios.solicitado_por, ".
                    ...
                    "servicios.estado_servicio_app, servicios.recorrido_gps, servicios.aceptado_app, ".
                    "servicios.aerolinea, servicios.vuelo, servicios.hora_salida, servicios.hora_llegada, ".
                    "servicios.cancelado, servicios.ver, servicios.localidad, pago_proveedores.consecutivo as pconsecutivo, pagos.autorizado, ".
                    ...
                    "where servicios.fecha_servicio between '".$fecha_inicial."' AND '".$fecha_final."' and servicios.localidad = 1 and servicios.ruta is not null";
                ...
                if($servicio!=0){
                    ...
                    //FILTRO PARA LOS SERVICIOS HABILITADOS POR FACTURACIÓN
                    if($servicio===10){
                        $consulta .=" and servicios.ver = 1";
                    }

                    if($servicio===11){
                        $consulta .=" and servicios.aceptado_app = 0";
                    }
                }else if($id_rol==33){
                    $consulta .= " and facturacion.facturado is null";
                }
    ...
    public function postMostrarsubcentros(){
      ...
      if (!Sentry::check()){
        ...
        if(Request::ajax()){
                ...
                $subcentros = Subcentro::nobloqueado()
                ->select('id', 'nombresubcentro', 'centrosdecosto_id')
                ->where('centrosdecosto_id',Input::get('centrosdecosto_id'))
                ->orderBy('nombresubcentro')->get();


            $nombre_rutas = DB::table('nombre_ruta')
            ->select('id', 'nombre', 'centrodecosto_id')
            ->where('centrodecosto_id', Input::get('centrosdecosto_id'))
            ->get();
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()) {
                ...
                if ($validador->fails()){
                    ...
                    if (Sentry::getUser()->id_rol==1 or Sentry::getUser()->id_rol==2 or Sentry::getUser()->id==495 or Sentry::getUser()->id==68 or Sentry::getUser()->id==37 or Sentry::getUser()->id==170) {
                      ...
                      if ($servicios->save()) {
                        ...
                        $pasajeroeditado = DB::table('qr_rutas')
                          ...
                          ->update([
                            'conductor_id' => $servicios->conductor_id,
                          ...
                          if( ($conductor!=$servicios->conductor_id) || ($fecha_servicio!=$servicios->fecha_servicio) || ($hora_servicio!=$servicios->hora_servicio) ){ //Si hay un cambio de CONDUCTOR / FECHA / HORA

                            /*Alerta por whatsapp a los pasajeros*/
                              ...
                              if($pasajeros){
                                ...
                                foreach ($pasajeros as $pasajero) {
                                    ...
                                    $code = $pasajero->id_usuario;

                                    $fecha = Input::get('fecha_servicio');
                                    $hora = str_replace(':', '-', Input::get('hora_servicio'));
                                    $data = $code.$fecha.$hora.$servicios->id.$servicios->conductor_id; //Valor de Código QR - CONCATENACIÓN

                                    if($pasajero->cel!=null){ //Tiene número

                                        //Inicio generación de código QR
                                        $qr_code = DNS2D::getBarcodePNG($data, "QRCODE", 10, 10,array(1,1,1), true); //Creación de QR
                                        Image::make($qr_code)->save('biblioteca_imagenes/qr_codes/'.$data.'.png'); //Creación del QR P2
                                        //Fin generación de código QR

                                        $fullname = trim($pasajero->fullname);
                                        $namewaap = explode(" ", $fullname);

                                        //NUEVO
                                        $qr = Crypt::encryptString($data); //Encryptación del valor de QR
                                        $phone = $pasajero->cel;
                                        $number = '57'.$phone; //Concatenación del indicativo con el número
                                        ...
                                        $conductorVehiculo = DB::table('conductores')
                                    ...
                                    $actualizacion = DB::table('qr_rutas')
                                    ->where('servicio_id',$servicios->id)
                                    ->where('id_usuario',$pasajero->id_usuario)
                                    ->update([
                                        'code' => $data
                                    ]);
                                }
                          ...
                          if($vehiculo==0){
                            //$ruta_nombre_1 = Vehiculo::find($servicios->ruta_nombre_id);
                            $v = DB::table('vehiculos')->where('id',$servicios->vehiculo_id)->first();
                            $cambios = $cambios.'Se asigno el vehiculo de marca <b>'.$v->marca.'</b> y placas <b>'.$v->placa.'<br>';
                          }else if ($vehiculo!=$servicios->vehiculo_id) {
    ...
    public function postMostrarproveedores(){
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()) {
                $conductorIdArray = explode(',', Input::get('conductoresid'));
                $conductores = [];
                $array = [];
                for ($i=0; $i < count($conductorIdArray); $i++) {
                    $name = DB::table('conductores')->where('id',$conductorIdArray[$i])->pluck('nombre_completo');
                    $array[$i] = $name;
                }
                return Response::json([
                    'respuesta'=>true,
    ...
    /*public function postMostrarlocalidades(){
        if(Request::ajax()){
            $recogeren = DB::table('lugares')->where('tipo_lugar',1)->orderBy('nombre_lugar')->get();
            $dejaren = DB::table('lugares')->where('tipo_lugar',2)->orderBy('nombre_lugar')->get();
...
*/
    public function postMostrarconductoresyvehiculos(){
        ...
        if(Request::ajax()){
            ...
            $conductores = Conductor::bloqueado()
            ->select('id', 'nombre_completo', 'bloqueado', 'bloqueado_total', 'proveedores_id')->where('proveedores_id', Input::get('proveedores_id'))
            ->get();

            $vehiculos = Vehiculo::bloqueadototal()
            ->select('id', 'placa', 'clase', 'marca', 'modelo', 'ano', 'capacidad', 'color', 'proveedores_id', 'bloqueado_total', 'bloqueado', 'bloqueado_mantenimiento')
            ->bloqueado()->where('proveedores_id', Input::get('proveedores_id'))->get();
    ...
    public function postMostrardatosservicio(){
        ...
        $servicio = DB::table('servicios')
            ->select('id', 'fecha_orden', 'centrodecosto_id', 'subcentrodecosto_id', 'pasajeros', 'pasajeros_ruta', 'cantidad', 'departamento', 'ciudad', 'solicitado_por', 'fecha_solicitud', 'email_solicitante', 'ruta_id', 'expediente', 'recoger_en', 'desde', 'dejar_en', 'hasta', 'detalle_recorrido', 'proveedor_id', 'conductor_id', 'vehiculo_id', 'fecha_servicio', 'hora_servicio', 'resaltar', 'pago_directo', 'origen', 'destino', 'aerolinea', 'vuelo', 'hora_salida', 'hora_llegada', 'ruta', 'ruta_nombre_id') 
            ->where('id',Input::get('id'))
            ->first();

        $proveedor = DB::table('proveedores')
            ->select('id', 'razonsocial', 'localidad', 'eventual', 'inactivo')
            ...
            Excel::selectSheetsByIndex(0)->load(Input::file('excel'), function($reader) use ($nombres_ruta){
                ...
                foreach($result as $value){
                  ...
                  if(is_string($value[1])){
                    ...
                    if(Sentry::getUser()->id_rol == 8){
                        $areas = 'RUTA EJECUTADA CON ÉXITO';
                    }else{
                        $areas = strtoupper($value[6]);
                    }

                    $dataExcel = [
                       'nombres' => strtoupper(trim($nombre_excel)), //CEDULA
                       'apellidos' => strtoupper($apellidos_excel), //NOMBRES
                       'cedula' => trim($value[2]), //TELEFONO
                       'direccion' => strtoupper(trim($direccion)), //DIRECCION
                       'barrio' => strtoupper(trim($barrio)), //BARRIO
                       'cargo' => strtoupper($localidad), //LOCALIDAD
                       'area' => $areas, //CIUDAD
                       'sub_area' => strtoupper($campana), //PROGRAMA
                       'hora' => $value[8], //HORA
                       'ruta' => strtoupper(trim($value[9])), //RUTA
                       'namec' => strtoupper(trim($value[10])), //NOMBRE CONDUCTOR
                       'conductor' => trim($value[11]), //ID CONDUCTOR
                       'placa' => trim($value[12]), //PLACA
                       'recoger' => trim($value[13]), //R
                       'dejar' => trim($value[14]), //D
                    ];
        ...
        if (!Sentry::check()){
            ...
            for ($i=0; $i < count($rutaIdArray); $i++) {
                ...
                if($liquidacion!=null or $ordenes_facturacion!=null){
                  ...
                  $vehiculo = Vehiculo::where('placa', $vehiculoplacaArray[$i])->with(['restriccionvehiculo' => function ($query) {
                  ...
                  $fecha_vigencia_operacion = Vehiculo::calculoCantidadDias($vehiculo->fecha_vigencia_operacion,$fecha);
                  $fecha_vigencia_soat = Vehiculo::calculoCantidadDias($vehiculo->fecha_vigencia_soat,$fecha);
                  $fecha_vigencia_tecnomecanica = Vehiculo::calculoCantidadDias($vehiculo->fecha_vigencia_tecnomecanica,$fecha);
                  $mantenimiento_preventivo = Vehiculo::calculoCantidadDias($vehiculo->mantenimiento_preventivo,$fecha);
                  $poliza_contractual = Vehiculo::calculoCantidadDias($vehiculo->poliza_contractual,$fecha);
                  $poliza_extracontractual = Vehiculo::calculoCantidadDias($vehiculo->poliza_extracontractual,$fecha);
                  //
                  $sw = 0;
                  ...
                  if($fecha_vigencia_operacion<0){
                    $operacion = 1;
                    $sw = 1;
                  }

                  if($fecha_vigencia_soat<0){
                    $soat = 1;
                    $sw = 1;
                  }

                  if($fecha_vigencia_tecnomecanica<0){
                    $tecno = 1;
                    $sw = 1;
                  }
                  ...
                  $knows = DB::table('vehiculos')
                  ->where('id',$vehiculo->id)
                  ->first();

                  if($knows->bloqueado_mantenimiento==1){
                    $mante = 1;
                    $sw = 1;
                  }

                  if($knows->bloqueado==1 or $knows->bloqueado_total==1){
                    $juri = 1;
                    $sw = 1;
                  }
                  /*End Vehículo*/
                  ...
                  /*Start Conductor*/
                  $conductor = Conductor::find($conductorIdArray[$i]);

                  $dia_mes = date('d-');

                  $date = date('Y-m-d');

                  $seguridad_social = "select pago, fecha_inicial, fecha_final from seguridad_social where conductor_id = ".$conductorIdArray[$i]." and '".$date."' between fecha_inicial and fecha_final ";
                  ...
                  $cond_examenes = DB::table('conductor_examenes')->where('conductor_id',$conductorIdArray[$i])
                  ...
                  /*Star Condicionales Conductor*/
                  $conds = DB::table('conductores')
                  ->where('id',$conductor->id)
                  ->first();

                  if($conds->bloqueado==1 or $conds->bloqueado_total==1){
                    $juri2 = 1;
                    $sw = 1;
                  }
                  ...
                  //if($dias_examen_ultimo<= -365 or $dias_examen_ultimo==null){ //Solucionar
                  if(1>2){ //Solucionar
                    $exam = 1;
                    $sw = 1;
                  }
                  /*End Condicionales Conductor*/
                  ...
                  if($sw===1){
                    ...
                    $array = [
                      'ruta' => $detalleArray[$i],
                      'vehiculo' => $vehiculoplacaArray[$i],
                      'conductor' => $conductor->nombre_completo,
                      'administracion' => $admin,
                      'fecha_vigencia_operacion' => $operacion,
                      'fecha_vigencia_soat' => $soat,
                      'fecha_vigencia_tecnomecanica' => $tecno,
                      'mantenimiento_preventivo' => $preventiva,
                      'poliza_contractual' => $contra,
                      'poliza_extracontractual' => $extra,
                      'bloqueo_mantenimiento' => $mante,
                      'bloqueado' => $juri,
                      'seguridad_social' => $ss,
                      'licencia_conduccion' => $lic,
                      'examenes' => $dias_examen_ultimo,
                      'bloqueo_juridica' => $juri2
                    ];
            ...
            //if(3>4){
              ...
              ->where('id',Input::get('centrodecosto'))
              ->pluck('razonsocial');
              ...
              for ($i=0; $i < count($rutaIdArray); $i++) {
                  ...
                  if($liquidacion!=null or $ordenes_facturacion!=null){
                      ...
                      if($recoger_enArray[$i]=='AUTO OCCIDENTE' or $recoger_enArray[$i]=='AUTO CENTRO' or $recoger_enArray[$i]=='AUTO SUR OCCIDENTE' or $recoger_enArray[$i]=='AUTO NOR OCCIDENTE' or $recoger_enArray[$i]=='AUTO SUR' or $recoger_enArray[$i]=='AUTO NORTE'){
                        $reco = 'AUTO';

                        if($recoger_enArray[$i]=='AUTO OCCIDENTE'){
                            $servicio->ruta_id = 45;
                        }else if($recoger_enArray[$i]=='AUTO CENTRO'){
                            $servicio->ruta_id = 52;
                        }else if($recoger_enArray[$i]=='AUTO SUR OCCIDENTE'){
                            $servicio->ruta_id = 47;
                        }else if($recoger_enArray[$i]=='AUTO NOR OCCIDENTE'){
                            $servicio->ruta_id = 46;
                        }else if($recoger_enArray[$i]=='AUTO SUR'){
                            $servicio->ruta_id = 48;
                        }else if($recoger_enArray[$i]=='AUTO NORTE'){
                            $servicio->ruta_id = 49;
                        }else{
                            $servicio->ruta_id = 61;
                        }

                      }else if($recoger_enArray[$i]=='VAN OCCIDENTE' or $recoger_enArray[$i]=='VAN CENTRO' or $recoger_enArray[$i]=='VAN SUR OCCIDENTE' or $recoger_enArray[$i]=='VAN NOR OCCIDENTE' or $recoger_enArray[$i]=='VAN SUR' or $recoger_enArray[$i]=='VAN NORTE'){
                        $reco = 'VAN';

                        if($recoger_enArray[$i]=='VAN OCCIDENTE'){
                            $servicio->ruta_id = 45;
                        }else if($recoger_enArray[$i]=='VAN CENTRO'){
                            $servicio->ruta_id = 52;
                        }else if($recoger_enArray[$i]=='VAN SUR OCCIDENTE'){
                            $servicio->ruta_id = 47;
                        }else if($recoger_enArray[$i]=='VAN NOR OCCIDENTE'){
                            $servicio->ruta_id = 46;
                        }else if($recoger_enArray[$i]=='VAN SUR'){
                            $servicio->ruta_id = 48;
                        }else if($recoger_enArray[$i]=='VAN NORTE'){
                            $servicio->ruta_id = 49;
                        }else{
                            $servicio->ruta_id = 61;
                        }
                        ...
                        $reco = $recoger_enArray[$i];

                        if($recoger_enArray[$i]=='AUTO SOACHA' or $recoger_enArray[$i]=='VAN SOACHA'){
                            $servicio->ruta_id = 50;
                        }else if($recoger_enArray[$i]=='SABANA'){
                            $servicio->ruta_id = 53;
                        }else if($recoger_enArray[$i]=='AUTO USME' or $recoger_enArray[$i]=='VAN USME'){
                            $servicio->ruta_id = 51;
                        }else{
                            $servicio->ruta_id = 61;
                        }
                      ...
                      $servicio->recoger_en = $reco; //Nuevo guardar el dato en el campo normal
                      if( Input::get('centrodecosto')==287 and $recoger_enArray[$i]=='SUTHERLAND'){
                        if(Input::get('subcentrodecosto')==851){
                          $servicio->desde = '[{"lat":"4.629206186047305","lng":"-74.11143302261786"}]'; //AMERICAS
                        }else if(Input::get('subcentrodecosto')==853){
                          $servicio->desde = '[{"lat":"4.731655260896234","lng":"-74.02367455028008"}]'; //TK
                        }
                      }else if( Input::get('centrodecosto')==349 and $recoger_enArray[$i]=='OPTUM' ){
                        $servicio->desde = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //OPTUM
                      }else if( Input::get('centrodecosto')==489 and $recoger_enArray[$i]=='BURÓ 25' ) {
                        $servicio->desde = '[{"lat":"4.683052579646723","lng":"-74.1237429973836"}]'; //BURÓ 25
                      }else if( Input::get('centrodecosto')==489 and $recoger_enArray[$i]=='CONECTA' ) {
                        $servicio->desde = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //CONECTA
                      }else if( Input::get('centrodecosto')==323 and $recoger_enArray[$i]=='CARVAJAL' ) {
                        $servicio->desde = '[{"lat":"4.68137844607486","lng":"-74.11938422145148"}]'; //CARVAJAL
                      }else{
                        $servicio->desde = $recoger_enArray[$i]; //DATO ORIGINAL
                      }
                      //$servicio->desde = $recoger_enArray[$i]; //Nuevo Guardar el dato en el campo nuevo

                      if($dejar_enArray[$i]=='AUTO OCCIDENTE' or $dejar_enArray[$i]=='AUTO CENTRO' or $dejar_enArray[$i]=='AUTO SUR OCCIDENTE' or $dejar_enArray[$i]=='AUTO NOR OCCIDENTE' or $dejar_enArray[$i]=='AUTO SUR' or $dejar_enArray[$i]=='AUTO NORTE'){
                        $deja = 'AUTO';

                        if($dejar_enArray[$i]!='SUTHERLAND') {
                            if($dejar_enArray[$i]=='AUTO OCCIDENTE'){
                                $servicio->ruta_id = 45;
                            }else if($dejar_enArray[$i]=='AUTO CENTRO'){
                                $servicio->ruta_id = 52;
                            }else if($dejar_enArray[$i]=='AUTO SUR OCCIDENTE'){
                                $servicio->ruta_id = 47;
                            }else if($dejar_enArray[$i]=='AUTO NOR OCCIDENTE'){
                                $servicio->ruta_id = 46;
                            }else if($dejar_enArray[$i]=='AUTO SUR'){
                                $servicio->ruta_id = 48;
                            }else if($dejar_enArray[$i]=='AUTO NORTE'){
                                $servicio->ruta_id = 49;
                            }else{
                                $servicio->ruta_id = 61;
                            }
                      ...
                      }else if($dejar_enArray[$i]=='VAN OCCIDENTE' or $dejar_enArray[$i]=='VAN CENTRO' or $dejar_enArray[$i]=='VAN SUR OCCIDENTE' or $dejar_enArray[$i]=='VAN NOR OCCIDENTE' or $dejar_enArray[$i]=='VAN SUR' or $dejar_enArray[$i]=='VAN NORTE'){
                        $deja = 'VAN';

                        if($dejar_enArray[$i]!='SUTHERLAND') {
                            if($dejar_enArray[$i]=='VAN OCCIDENTE'){
                                $servicio->ruta_id = 45;
                            }else if($dejar_enArray[$i]=='VAN CENTRO'){
                                $servicio->ruta_id = 52;
                            }else if($dejar_enArray[$i]=='VAN SUR OCCIDENTE'){
                                $servicio->ruta_id = 47;
                            }else if($dejar_enArray[$i]=='VAN NOR OCCIDENTE'){
                                $servicio->ruta_id = 46;
                            }else if($dejar_enArray[$i]=='VAN SUR'){
                                $servicio->ruta_id = 48;
                            }else if($dejar_enArray[$i]=='VAN NORTE'){
                                $servicio->ruta_id = 49;
                            }else{
                                $servicio->ruta_id = 61;
                            }
                        ...
                        $deja = $dejar_enArray[$i];

                        if($dejar_enArray[$i]!='SUTHERLAND' and $dejar_enArray[$i]!='OPTUM' and $dejar_enArray[$i]!='BURÓ 25' and $dejar_enArray[$i]!='CONECTA' and $dejar_enArray[$i]!='CARVAJAL' and $dejar_enArray[$i]!='BANCO POPULAR' and $dejar_enArray[$i]!='EMERGIA') {
                            if($dejar_enArray[$i]=='AUTO SOACHA' or $dejar_enArray[$i]=='VAN SOACHA'){
                                $servicio->ruta_id = 50;
                            }else if($dejar_enArray[$i]=='SABANA'){
                                $servicio->ruta_id = 53;
                            }else if($dejar_enArray[$i]=='AUTO USME' or $dejar_enArray[$i]=='VAN USME'){
                                $servicio->ruta_id = 51;
                            }else{
                                $servicio->ruta_id = 61;
                            }
                        }else{
                          $servicio->tipo_ruta = 1;
                        }

                      }
                      $servicio->dejar_en = $deja; //Nuevo guardar el dato en el campo normal
                      if( Input::get('centrodecosto')==287 and $dejar_enArray[$i]=='SUTHERLAND'){
                        if(Input::get('subcentrodecosto')==851){
                          $servicio->hasta = '[{"lat":"4.629206186047305","lng":"-74.11143302261786"}]'; //AMERICAS
                        }else if(Input::get('subcentrodecosto')==853){
                          $servicio->hasta = '[{"lat":"4.731655260896234","lng":"-74.02367455028008"}]'; //TK
                        }
                      }else if( Input::get('centrodecosto')==349 and $dejar_enArray[$i]=='OPTUM' ) {
                        $servicio->hasta = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //OPTUM
                      }else if( Input::get('centrodecosto')==489 and $dejar_enArray[$i]=='BURÓ 25' ) {
                        $servicio->hasta = '[{"lat":"4.683052579646723","lng":"-74.1237429973836"}]'; //BURÓ 25
                      }else if( Input::get('centrodecosto')==489 and $dejar_enArray[$i]=='CONECTA' ) {
                        $servicio->hasta = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //CONECTA
                      }else if( Input::get('centrodecosto')==323 and $dejar_enArray[$i]=='CARVAJAL' ) {
                        $servicio->hasta = '[{"lat":"4.68137844607486","lng":"-74.11938422145148"}]'; //CARVAJAL
                      }else{
                        $servicio->hasta = $dejar_enArray[$i]; //Nuevo
                      }
                      //$servicio->hasta = $dejar_enArray[$i]; //Nuevo Guardar el dato en el campo nuevo
                      ...
                      if($placa!=null){
                          ...
                          $placa2 = Vehiculo::where('proveedores_id',$proveedor)->whereNull('bloqueado')->whereNull('bloqueado_total')->first();
                          $servicio->vehiculo_id = $placa2->id;
                      }
                      ...
                      $servicio->localidad = 1; //CONTROL PARA VISTA DE BOGOTÁ
                      $servicio->aceptado_app = 0;
                      $servicio->hora_programado_app = date('Y-m-d H:i:s');

                      $rnid = DB::table('nombre_ruta')
                      ->where('nombre',($detalleArray[$i]))
                      ->pluck('id');
                      ...
                      $test = '';
                        ...
                        if(Input::get('fecha_servicio') == date('Y-m-d')){
                            $dia = 'HOY';
                        }else if(Input::get('fecha_servicio') == $manana){
                            $dia = 'MANANA';
                        }else{
                            $dia = Input::get('fecha_servicio');
                        }

                      foreach ($pasajeros[$i] as $item){

                        $code = intval(trim($item->apellidos));//Employ ID enviado por SGS
                        $phone = trim($item->cedula); //Celular del pasajero

                        $fecha = Input::get('fecha_servicio');
                        $hora = str_replace(':', '-', $horaArray[$i]);
                        $data = $code.$fecha.$hora.$servicio->id.$servicio->conductor_id; //Valor de Código QR - CONCATENACIÓN

                        $address = trim($item->direccion);
                        $neighborhood = trim($item->barrio);
                        ...
                        $usuarioRuta = new Ruta_qr();
                        $usuarioRuta->id_usuario = $code;
                        $usuarioRuta->code = $data;
                        $usuarioRuta->cel = $phone;
                        $usuarioRuta->fecha = Input::get('fecha_servicio');
                        $usuarioRuta->hora = $horaArray[$i];
                        $usuarioRuta->vehiculo=$vehiculoplacaArray[$i];
                        $usuarioRuta->servicio_id = $servicio->id;
                        $usuarioRuta->fullname = trim($item->nombres);
                        $usuarioRuta->address = $address;
                        //$usuarioRuta->locality = 'Localidad del pasajero'; //Si aplica
                        $usuarioRuta->neighborhood = $neighborhood;
                        $usuarioRuta->coords = json_encode([$coords]);
                        if(Sentry::getUser()->id_rol == 8){
                            $usuarioRuta->id_rutaqr = $item->area;
                        }
                        $usuarioRuta->save();
                        ...
                        if( (date('Y-m-d') < $servicio->fecha_servicio) || (date('Y-m-d') == $servicio->fecha_servicio and date('H:i')<= $horaMasTreita ) ){ //Input::get('mensajesqr')
                        ...
                        } //FIN SI SE SELECCIONÓ LA OPCIÓN DE ENVÍO DE MENSAJES POR WA

                      }



                  }//RESTRICCION LIQUIDACION
              ...
              return Response::json([
                  ...
                  'contador'=>$contador,
                  'contadora' => $contadora,
                  'cc'=>Input::get('centrodecosto'),
                  'sub'=>Input::get('subcentrodecosto'),
    ...
    //new
    public function postNuevaruta2(){
        ...
        if (!Sentry::check()){
            ...
            $conductorIdArray = explode(',', Input::get('conductorArray'));
            $vehiculoIdArray = explode(',', Input::get('vehiculoArray'));
            $recoger_enArray = explode(',', Input::get('recoger_enArray'));
            $dejar_enArray = explode(',', Input::get('dejar_enArray'));
            $horaArray = explode(',', Input::get('horaArray'));
            ...
            $objArray = [];
              ...
              for ($i=0; $i < count($rutaIdArray); $i++) {
                  ...
                  if($liquidacion!=null or $ordenes_facturacion!=null){
                      ...
                      $servicio->departamento = 'CUNDINAMARCA';//Input::get('departamento');
                      $servicio->ciudad = 'BOGOTA';//Input::get('ciudad');
                      ...
                      $servicio->recoger_en = $reco; //Nuevo guardar el dato en el campo normal
                      if( Input::get('centrodecosto')==287){
                        if(Input::get('subcentrodecosto')==851){
                          $servicio->desde = '[{"lat":"4.629206186047305","lng":"-74.11143302261786"}]'; //AMERICAS
                        }else if(Input::get('subcentrodecosto')==853){
                          $servicio->desde = '[{"lat":"4.731655260896234","lng":"-74.02367455028008"}]'; //TK
                        ...
                        $servicio->desde = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //OPTUM
                      }else if( Input::get('centrodecosto')==489 and $recoger_enArray[$i]=='BURÓ25' ) {
                        $servicio->desde = '[{"lat":"4.683052579646723","lng":"-74.1237429973836"}]'; //BURÓ 25
                      }else if( Input::get('centrodecosto')==489 and $recoger_enArray[$i]=='CONECTA' ) {
                        $servicio->desde = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //CONECTA
                      }else if( Input::get('centrodecosto')==323 and $recoger_enArray[$i]=='CARVAJAL' ) {
                      ...
                      if($dejar_enArray[$i]=='AUTO OCCIDENTE' or $dejar_enArray[$i]=='AUTO CENTRO' or $dejar_enArray[$i]=='AUTO SUR OCCIDENTE' or $dejar_enArray[$i]=='AUTO NOR OCCIDENTE' or $dejar_enArray[$i]=='AUTO SUR' or $dejar_enArray[$i]=='AUTO NORTE'){
                        ...
                        if($dejar_enArray[$i]!='SUTHERLAND' and $dejar_enArray[$i]!='SUTHERLAND AMERICAS' and $dejar_enArray[$i]!='SUTHERLAND TORRE KRYSTAL' and $dejar_enArray[$i]!='OPTUM' and $dejar_enArray[$i]!='BURÓ25' and $dejar_enArray[$i]!='CONECTA' and $dejar_enArray[$i]!='CARVAJAL' and $dejar_enArray[$i]!='BANCO POPULAR' and $dejar_enArray[$i]!='EMERGIA') {
                            if($dejar_enArray[$i]=='AUTO SOACHA' or $dejar_enArray[$i]=='VAN SOACHA'){
                                $servicio->ruta_id = 50;
                            }else if($dejar_enArray[$i]=='SABANA'){
                                $servicio->ruta_id = 53;
                      ...
                      if( Input::get('centrodecosto')==287){
                        ...
                        $servicio->hasta = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //OPTUM
                      }else if( Input::get('centrodecosto')==489 and $dejar_enArray[$i]=='BURÓ25' ) {
                        $servicio->hasta = '[{"lat":"4.683052579646723","lng":"-74.1237429973836"}]'; //BURÓ 25
                      }else if( Input::get('centrodecosto')==489 and $dejar_enArray[$i]=='CONECTA' ) {
                        $servicio->hasta = '[{"lat":"4.684278776619403","lng":"-74.11935600007445"}]'; //CONECTA
                      }else if( Input::get('centrodecosto')==323 and $dejar_enArray[$i]=='CARVAJAL' ) {
                      ...
                      //$servicio->desde = $recoger_enArray[$i]; //Nuevo
                      $servicio->recoger_en = $recoger_enArray[$i]; //Nuevo

                      //$servicio->hasta = $dejar_enArray[$i]; //Nuevo
                      $servicio->dejar_en = $dejar_enArray[$i]; //Nuevo
                      ...
                      $servicio->vehiculo_id = $vehiculoIdArray[$i];
                      $servicio->fecha_servicio = Input::get('fecha_servicio');
                      $servicio->hora_servicio = $horaArray[$i];
                      $servicio->resaltar = 0;
                      $servicio->pago_directo = null;
                      ...
                      foreach ($pasajeros[$i] as $item){
                        ...
                        $code = intval(trim($item->cedula));//Employ ID enviado por SGS
                        $phone = trim($item->apellidos); //Celular del pasajero

                        $fecha = Input::get('fecha_servicio');
                        $hora = str_replace(':', '-', $horaArray[$i]);
                        $data = $phone.$fecha.$hora.$servicio->id.$servicio->conductor_id; //Valor de Código QR - CONCATENACIÓN
                        ...
                        $usuarioRuta = new Ruta_qr();
                        $usuarioRuta->id_usuario = $phone;
                        $usuarioRuta->code = $data;
                        $usuarioRuta->cel = $code;
                        $usuarioRuta->fecha = Input::get('fecha_servicio');
                        $usuarioRuta->hora = $horaArray[$i];
                        $usuarioRuta->servicio_id = $servicio->id;
                        $usuarioRuta->fullname = trim($item->nombres);
                        $usuarioRuta->address = $address;
                        $usuarioRuta->locality = $item->cargo;
                        $usuarioRuta->neighborhood = $neighborhood;
                        $usuarioRuta->coords = json_encode([$coords]);
                        $usuarioRuta->save();
                        ...
                        if( (date('Y-m-d') < $servicio->fecha_servicio) || (date('Y-m-d') == $servicio->fecha_servicio and date('H:i')<= $horaMasTreita ) ){ //Input::get('mensajesqr')
                            ...
                            $number = '57'.$code; //Concatenación del indicativo con el número
                            ...
                            $nombre = $namewaap[0];
                            $hora = $horaArray[$i];
                            //$placa = 'UUW126'; //$vehiculoplacaArray[$i]; //
                            $conductor = $nameConductor;
                            $numero = $celConductor;
                            ...
                            ->where('id_empleado',$phone)
                            ->first();

                            if($user){
                              Servicio::rutaProgramada($user->idregistrationdevice,$user->idioma,$servicio,$placa);
                            }
                            /* Notificar APP */
                      ...
                      ->where('id',Input::get('id_solicitud'))
                      ->update([
                        'montado' => 1
                      ]);
    ...
    //if(Sentry::getUser()->id===2){
        ...
        if (!Sentry::check()){
            ...
            $json = json_decode(Input::get('pasajeros'));
            ...
            foreach ($json as $data) {
                $cont++;
                //BÚSQUEDA DEL ESTADO DEL USUARIO EN EL CICLO
                $search = DB::table('qr_rutas')
                ->where('employ',$data->apellidos)
                ->where('servicio_id',Input::get('id'))
                ->pluck('status');

                $search2 = DB::table('qr_rutas')
                ->where('employ',$data->apellidos)
                ->where('servicio_id',Input::get('id'))
                ->pluck('id_usuario');

                if($search2===null){
                    $us = DB::table('pasajeros')->where('id_employer',$data->apellidos)->pluck('cedula');
                    if($us){
                        $search2 = $us;
                    }else{
                        $search2 = $data->apellidos;
                    }
                }

                $qr = new Ruta_qr();
                $qr->employ = $data->apellidos;
                $qr->id_usuario = $search2;
                $qr->cel = $data->cedula;
                $qr->address = $data->direccion;
                $qr->fullname = $data->nombres;
                $qr->neighborhood = $data->barrio;
                $qr->locality = $data->cargo;
                $qr->centrodecosto_id = $servicio->centrodecosto_id;
                $qr->fecha = $servicio->fecha_servicio;
                $qr->hora = $servicio->hora_servicio;
                $qr->id_rutaqr = $servicio->detalle_recorrido;
                $qr->updated_at = date('Y-m-d H:i:s');
                $qr->servicio_id = Input::get('id');
                $qr->subcentrodecosto_id = 201920;
                //SI YA ESTABA EL USUARIO, SE AGREGA CON EL ESTADO QUE TENÍA.
                if($search){
                    $qr->status = $search;
                }else{ //SI EL USUARIO ES NUEVO AGREGADO, SE CONSULTA SI EL USUARIO ESTÁ REGISTRADO PARA QUE SE PUEDA LEER SU CÓDIGO.

                    $user = DB::table('pasajeros')
                    ->where('id_employer',$data->apellidos)
                    ->pluck('id_employer');
                    //SI ESTÁ REGISTRADO, SE COLOCA NULL: USUARIO REGISTRADO PARA LEER.
                    if($user){
                        $qr->status = null;
                    }else{ //SI NO ESTÁ REGISTRADO, SE COLOCA ESTADO 3.
                        $qr->status = 3;
                    }
                }

                //AVISAR AL USUARIO ELIMINADO QUE ESTA YA NO SERÁ SU RUTA

                //ENVIAR MENSAJE AL USUARIO NUEVO (COLOCAR UN CHECK PARA ELEGIR; PREDETERMINADO, NO.)
                $qr->save();
            }

            //ELIMINACION DE LOS USUARIOS DE ESE SERVICIO
            $pasajeros_eliminados = DB::table('qr_rutas')
            ->where('servicio_id', Input::get('id'))
            ->where('subcentrodecosto_id','!=',201920)
            ->delete();
            if($pasajeros_eliminados){
                $sw2 = 'si';
            }else{
                $sw2 = 'no';
            }
            $sw = 0;
            //AVISO POR MENSAJE A LOS USUARIOS QUE SEAN ELIMINADOS DE LA RUTA
            foreach ($pasajeros_search as $pass) {

                $consulta_sms = DB::table('qr_rutas')
                ->where('servicio_id',Input::get('id'))
                ->where('id_usuario',$pass->id_usuario)
                ->get();

                //SI EL USUARIO ES ELIMINADO, SE LE ENVÍA EL MENSAJE DE AVISO.
                if(!$consulta_sms){
                    $sw = 1;
                    $namesms = explode(" ", $pass->fullname);

                    $post['to'] = array('57'.$pass->cel);
                    $post['text'] = "AOTOUR le indica sr(a) ".$namesms[0].", que su ruta ya no es ".$servicio->detalle_recorrido." el ".$servicio->fecha_servicio." a las ".$servicio->hora_servicio.". Pronto recibirá los datos de su ruta.";
                    $post['from'] = "msg";
                    $user ="Aotour";
                    $password = 'Tour20+';
                    $ch = curl_init();
            ...
            //ACTULIZACIÓN DE SUBCENTRODECOSTO_ID
            $subcentro_update = DB::table('qr_rutas')
            ->where('servicio_id', Input::get('id'))
            ->where('subcentrodecosto_id',201920)
            ->update([
            ...
            if($contador_actual!=$cont){

                $celular_conductor = DB::table('conductores')->where('id',$servicio->conductor_id)->pluck('celular');
                $post['to'] = array('57'.$celular_conductor);
                $post['text'] = "AOTOUR le informa que su ruta ".$servicio->detalle_recorrido." del ".$servicio->fecha_servicio.", a las ".$servicio->hora_servicio." tuvo un cambio en los pasajeros que se van a transportar. Revise su lista de usuarios en la app.";
                $post['from'] = "msg";
                $user ="Aotour";
                $password = 'Tour20+';
                $ch = curl_init();
...
*/
    ...
    public function postBuscarpapelera(){
              ...
              if (Request::ajax()) {
                  ...
                  $mensajes = [
                      ...
                      'pago_directo.numeric'=>'Debe seleccionar la opcion pago_directo',
  										...
  										'firma.numeric' => 'Debe seleccionar la opcion firma1',
      ...
      public function postHabilitarcoordinadores(){
            ...
            if(!Sentry::check()){
                ...
                DB::table('servicios')->where('id',$id)
                    ->update([
                            'ver'=>1,
                        ]);

                //$consulta = Servicio::where('id',$id)->get();
                //$consulta->ver = 1;
                //if($consulta->save()){
                    return Response::json([
                        'respuesta'=>true
      ...
      public function postDeshabilitarcoordinadores(){
