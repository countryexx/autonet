...
class TransportesrutasController extends BaseController{
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()){
                ...
                $consulta = "select servicios.id, servicios.id as servicioid, servicios.fecha_orden, servicios.fecha_servicio, servicios.solicitado_por, ".
                    ...
                    "servicios.estado_servicio_app, servicios.aceptado_app, servicios.recorrido_gps, ".
                    "servicios.aerolinea, servicios.vuelo, servicios.hora_salida, servicios.hora_llegada, ".
                    "servicios.cancelado, servicios.ruta_qr, pago_proveedores.consecutivo as pconsecutivo, pagos.autorizado, ".
                    "servicios_edicion_pivote.id as servicios_id_pivote, servicios.control_facturacion, servicios.estado_rechazo, ordenes_facturacion.id as idordenfactura, ".
                    ...
                    //MODIFICICACIÓN PARA QUE OPERACIONES NO VISUALICE LOS SERVICIOS MONTADOS POR FACTURACION
                    //SE AGREGÓ EL CAMPO CONTROL FACTURACION
                    "where servicios.fecha_servicio between '".$fecha_inicial."' AND '".$fecha_final."' and servicios.localidad is null and servicios.ruta is not null";
    ...
    public function postMostrarsubcentros(){
      ...
      if (!Sentry::check()){
        ...
        if(Request::ajax()){
            ...
            $fechaInicial = null;
            $fechaFinal = null;
            if(Input::get('centrosdecosto_id')==329){
              ...
              $fechaInicial = strtotime ('-85 day', strtotime($fechaInicial));
              $fechaInicial = date('Y-m-d' , $fechaInicial);

              $fechaFinal = date('Y-m-d');
              $fechaFinal = strtotime ('+20 day', strtotime($fechaFinal));
              $fechaFinal = date('Y-m-d' , $fechaFinal);

              //Consulta de los expedientes revisados
              $expedientes = "SELECT DISTINCT servicios.expediente, servicios.subcentrodecosto_id, servicios.localidad FROM servicios LEFT JOIN facturacion ON servicios.id = facturacion.servicio_id WHERE servicios.centrodecosto_id = 329 and servicios.expediente is not null and facturacion.revisado IS NOT NULL AND facturacion.liquidacion_id IS NULL AND servicios.fecha_servicio BETWEEN '".$fechaInicial."' AND '".$fechaFinal."' AND servicios.pendiente_autori_eliminacion IS null";

              $consulta = DB::select($expedientes);
    ...
    public function postConsultarfechas(){
      ...
      if (!Sentry::check()){
        ...
        if(Request::ajax()){
            ...
            if($expediente!='-'){
                ...
                //Consultar servicios sin revisar
                $sinRevisar = DB::table('servicios')
                ->leftJoin('facturacion', 'facturacion.servicio_id', '=', 'servicios.id')
                ->select('servicios.id', 'servicios.localidad')
                ->where('centrodecosto_id',329)
                ->where('expediente',$expediente)
                ->whereNull('facturacion.id')
                ->whereNull('servicios.pendiente_autori_eliminacion')
                ->get();
                ...
                if($sinRevisar){
                    ...
                    return Response::json([
                        'respuesta' => false,
                        'cantidad' => count($sinRevisar),
                        'servicios' => $sinRevisar
                    ]);
                    ...
                    ->leftJoin('facturacion', 'facturacion.servicio_id', '=', 'servicios.id')
                    ->select('servicios.fecha_servicio', 'servicios.id', 'facturacion.liquidacion_id', 'servicios.expediente', 'servicios.pendiente_autori_eliminacion')
                    ...
                    if($cantidad==1){
                        $fecha_inicial = $servicios[0]->fecha_servicio;
                        $fecha_final = $servicios[0]->fecha_servicio;
                    }else{
                        $inicio = 0;
                        foreach ($servicios as $servicio) {
                            if($inicio == 0){
                                $fecha_inicial = $servicio->fecha_servicio;
                            }
                            $inicio++;
                            if($inicio == $cantidad){
                                $fecha_final = $servicio->fecha_servicio;
                            }
                        }
                    }
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()) {
                ...
                if ($validador->fails()){
                    ...
                    if (Sentry::getUser()->id_rol==1 or Sentry::getUser()->id==37 or Sentry::getUser()->id==170) {
                        ...
                        $pasajeroeditado = DB::table('qr_rutas')
                          ...
                          if ($ruta_nombre==0 or $ruta_nombre==null) {
                            ...
                            if ($ruta_nombre != $servicios->ruta_nombre_id) {
                                ...
                                if(isset($servicios->ruta_nombre_id)){
    ...
    public function postMostrarproveedores(){
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()) {
                ...
                $proveedores = DB::select('select * from proveedores where inactivo is null and localidad in ("barranquilla","provisional") order by razonsocial');
                ...
                for($i=0; $i<$cantidadArrays; $i++){
                    ...
                    if($ordenes_facturacion!=null or $liquidacion!=null){
                        ...
                        if($servicio->proveedor_id!=45 and $servicio->proveedor_id!=336){
                            ...
                            $post['text'] = "Sr(a) ".$name_first[0].", se le ha programado un servicio para el dia ".$servicio->fecha_servicio.", a las ".$servicio->hora_servicio.". Por favor, revise su APP!";
                            $post['from'] = "msg";
                            $user ="Aotour";
                            $password = 'Tour20+';
                            $ch = curl_init();
        ...
        if (!Sentry::check()){
            ...
            for ($i=0; $i < count($rutaIdArray); $i++) {
                ...
                if($liquidacion!=null or $ordenes_facturacion!=null){
                    ...
                    $servicio->recoger_en = strtoupper(strtolower($recoger_enArray[$i]));
                    $servicio->dejar_en = strtoupper(strtolower($dejar_enArray[$i]));
                    ...
                    $servicio->ruta_nombre_id = $rutaIdArray[$i];
                    $servicio->save();

					$number = rand(10000000, 99999999);
                    //NOTIFICACIÓN DE ALERTA AL CONDUCTOR
                    ...
                    foreach ($pasajeros[$i] as $item){
                        ...
                        $code = intval(trim($item->cedula));

                        //Código que genera el QR del pasakerp
                        ...
                        $data = $valor1.$valor2; //Valor de Código QR
                        ...
                        /*if($pasajeroid!=null){
                            ...
                            $programacion_rutaqr->id_usuario=$pasajeroid->id_employer;
                            $programacion_rutaqr->vehiculo=$vehiculoArray[$i];
                            $programacion_rutaqr->conductor_id=$conductorArray[$i];
                            ...
                            $programacion_rutaqr->id_rutaqr=$detalleArray[$i];
                            $programacion_rutaqr->servicio_id = $servicio->id;
                            $programacion_rutaqr->fullname = $name.' '.$apel;
                            $programacion_rutaqr->address = trim($item->direccion);
                            $programacion_rutaqr->neighborhood = trim($item->barrio);
                            ...
                            $programacion_rutaqr = new Ruta_qr();
                            $programacion_rutaqr->id_usuario=$code;
                            $programacion_rutaqr->vehiculo=$vehiculoArray[$i];
                            $programacion_rutaqr->conductor_id=$conductorArray[$i];
                            $programacion_rutaqr->centrodecosto_id=Input::get('centrodecosto');
                            $programacion_rutaqr->subcentrodecosto_id=Input::get('subcentrodecosto');
                            $programacion_rutaqr->fecha= Input::get('fecha_servicio');
                            $programacion_rutaqr->hora=$horaArray[$i];
                            $programacion_rutaqr->id_rutaqr=strtoupper(strtolower($detalleArray[$i]));
                            $programacion_rutaqr->servicio_id = $servicio->id;
                            $programacion_rutaqr->status = 3;
                            $programacion_rutaqr->fullname = $name.' '.$apel;
                            $programacion_rutaqr->address = trim($item->direccion);
                            $programacion_rutaqr->neighborhood = trim($item->barrio);
                            $programacion_rutaqr->save();
        ...
        if (!Sentry::check()){
            ...
            foreach ($json as $data) {
                ...
                if($sw==1){
                    ...
                    //actualizar registro en qr ruta

                    $programa = strtoupper(trim($data->sub_area));
                    $programa = str_replace('AMP;', '', $programa);
                    $programa = str_replace('amp;', '', $programa);

                    $update = DB::table('qr_rutas')
                    ->where('servicio_id',Input::get('id'))
                    ->where('id_usuario',$data->apellidos)
                    ->update([
                        'id_rutaqr' => trim($data->area),
                        'employ' => $programa,
                        'fullname' => trim($data->nombres),
                        'address' => trim($data->direccion),
                        'cel' => $data->cedula,
                        'neighborhood' => trim($data->barrio)
                    ]);
                    ...
                    $qr->vehiculo = $placa;
                    $qr->id_rutaqr = $data->area;
                    if($data->sub_area!=null){
                        $qr->employ = $data->sub_area;
                    }
                    $qr->save();*/
                    ...
                    $qr->servicio_id = Input::get('id');
                    $qr->vehiculo = $placa;
                    $qr->id_rutaqr = trim($data->area);
                    if($data->sub_area!=null){
                        $qr->employ = trim($data->sub_area);
                    }
                    $qr->save();
