    ...
    public function getIndex(){
        ...
        if (!Sentry::check()){
            ...
            $query = DB::table('servicios')
                ...
                //->whereIn('servicios.subcentrodecosto_id', [825, 846, 847, 848])
        ...
        if (!Sentry::check()){
            ...
            if (Request::ajax()){
                ...
                $tipo_usuario = Sentry::getUser()->id_tipo_usuario;
                    ...
                    "servicios.cancelado, servicios.ver, pago_proveedores.consecutivo as pconsecutivo, pagos.autorizado, ".
                    ...
                    "where servicios.fecha_servicio between '".$fecha_inicial."' AND '".$fecha_final."' and users.localidad in (2,3) and servicios.ruta is not null and centrosdecosto.localidad in ('bogota','provisional') ";
                    //MODIFICICACIÓN PARA QUE OPERACIONES NO VISUALICE LOS SERVICIOS MONTADOS POR FACTURACION
                    //SE AGREGÓ EL CAMPO CONTROL FACTURACION
                    "where servicios.fecha_servicio between '".$fecha_inicial."' AND '".$fecha_final."' and users.localidad in (1,3) and servicios.ruta is not null and centrosdecosto.localidad in ('barranquilla','provisional') ";                    
                    if(!($id_rol==1 or $id_rol==2 or $id_rol==8)){
                ...
                foreach($result as $value){
                  ...
                  if(is_string($value[1])){
                    ...
                    $dataExcel = [
                       ...
                       'cargo' => strtoupper($value[5]), //LOCALIDAD
                       'area' => strtoupper($value[6]), //CIUDAD
                       'sub_area' => strtoupper($value[7]), //PROGRAMA
                       'hora' => $value[8], //HORA
        ...
        if (!Sentry::check()){
            ...
            for ($i=0; $i < count($rutaIdArray); $i++) {
                ...
                if($liquidacion!=null or $ordenes_facturacion!=null){
                    ...
                    $servicio->save();
                    //$consulta_sms = Conductor::find($conductorIdArray[$i])->get();
                    $consulta_sms = DB::table('conductores')->where('id',$conductorIdArray[$i])->pluck('celular');
                    $consulta_name = DB::table('conductores')->where('id',$conductorIdArray[$i])->pluck('nombre_completo');
                    $numberok = $consulta_sms;
                    $name_first = explode(" ", $consulta_name);
                    //ENVIO DE MENSAJE CONDUCTOR
                     //$nov = 'rutasbogota@aotour.com.co';
                    if($servicio->proveedor_id!=45 and $servicio->proveedor_id!=336){
                         $post['to'] = array('57'.$numberok);
                          $post['text'] = "Sr(a) ".$name_first[0].", se le ha programado la ruta ".strtoupper(strtolower($detalleArray[$i]))." para el dia ".$servicio->fecha_servicio.", a las ".$servicio->hora_servicio.". Por favor, revise su APP!";
                          $post['from'] = "msg";
                          $user ="Aotour";
                          $password = 'Tour20+';
                          $ch = curl_init();
                    ...
                    foreach ($pasajeros[$i] as $item){
                        $phone = trim($item->cedula);
                        $code = trim($item->apellidos);
                        $fullname = trim($item->nombres);
                        $address = trim($item->direccion);
                        $neighborhood = trim($item->barrio);
                        $locality = trim($item->cargo);

                        $pasajeroid = Pasajero::where('id_employer', $code)->first();                    
                        //$telefono = DB::table('pasajeros')->where('id_employer',$code)->pluck('telefono');
                        if($pasajeroid!=null){   
                            $usuarioqr = new Ruta_qr();
                            $usuarioqr->id_usuario=$pasajeroid->cedula;
                            $usuarioqr->cel=$phone;
                            $usuarioqr->vehiculo=$vehiculoplacaArray[$i];
                            $usuarioqr->conductor_id=$conductorIdArray[$i];
                            $usuarioqr->centrodecosto_id=Input::get('centrodecosto');
                            $usuarioqr->subcentrodecosto_id=Input::get('subcentrodecosto');
                            $usuarioqr->fecha= Input::get('fecha_servicio');
                            $usuarioqr->hora=$horaArray[$i];
                            $usuarioqr->id_rutaqr=strtoupper(strtolower($detalleArray[$i]));
                            $usuarioqr->servicio_id = $servicio->id;
                            $usuarioqr->fullname = $fullname;
                            $usuarioqr->address = $address;
                            $usuarioqr->locality = $locality;
                            $usuarioqr->neighborhood = $neighborhood;

                            $usuarioqr->save();  

                            //$name = trim($item->nombres);
                            //$apel = trim($item->apellidos);
                            //$namesms = explode(" ", $name);
                            //$apelsms = explode(" ", $apel);                            
                            //echo $porciones[0]; // porción1
                            $namesms = explode(" ", $fullname);
                            $nov = 'rutasbogota@aotour.com.co';
                             $post['to'] = array('57'.$phone);
                              $post['text'] = "AOTOUR le indica sr(a) ".$namesms[0].", su ruta es ".strtoupper(strtolower($detalleArray[$i]))." el ".Input::get('fecha_servicio')." a las ".$horaArray[$i]." en la sede ".$nombrerazonsocialsub.". Numero del conductor: ".$numberok."";
                              $post['from'] = "msg";
                              $user ="Aotour";
                              $password = 'Tour20+';
                              $ch = curl_init();
                            ...
                            $usuarioqr->hora=$horaArray[$i];
                            if($rnid!=null){
                                $usuarioqr->id_rutaqr=$rnid;//$rutaIdArray[$i];
                            }else{
                                $usuarioqr->id_rutaqr=strtoupper(strtolower($detalleArray[$i]));//$rutaIdArray[$i];
                            }
                            $usuarioqr->servicio_id = $servicio->id;
                            $usuarioqr->status = 3;
                            $usuarioqr->fullname = $fullname;
