...
<head>
    <meta charset="UTF-8">
    <title>Autonet | Rutas para programar Bog</title>
...
<div class="col-xs-12">
    ...
    <div class="col-lg-12">
      <div class="row">
        <h3 class="h_titulo">RUTAS PARA PROGRAMAR BOG</h3>
      </div>
    </div>
          ...
          @foreach($usuarios as $usuario)
            <tr>
              ...
              <td>
                ...
                <button data-id="{{$usuario->id}}" type="button" class="btn btn-info btn-list-table notify" @if($usuario->notificado==1 or $usuario->montado==1){{'disabled'}}@endif>
                  Notificar<i class="fa fa-bell"></i>
                </button>

              </td>
              <td>
                @if($usuario->montado!=1)
                  <a class="btn btn-primary btn-list-table" href="{{url('reportes/programacionderutas/'.$usuario->id)}}">PROGRAMAR <i class="fa fa-external-link" aria-hidden="true"></i></a>
                @else
                  <a class="btn btn-success btn-list-table" disabled>PROGRAMADO <i class="fa fa-check" aria-hidden="true"></i></a>
                @endif
              </td>
              <td>
                @if($usuario->descargado==1)
                  <center><div class="product-cell status-cell">
                    <span style="background: green; color: white; width: 120px" class="status active">Descargado ({{$usuario->cantidad}})</span>
                  </div></center>
                @else
                <center>
                  <button data-id="{{$usuario->id}}" type="button" class="btn btn-warning btn-list-table ocultar" @if($usuario->montado!=1){{'disabled'}}@endif>Ocultar<i class="fa fa-eye-slash"></i></button>
                </center>
                @endif
              </td>
            </tr>
...
<script type="text/javascript">
    ...
    $('.notify').click(function() {
      ...
      $.confirm({
          title: 'Confirmación',
          content: 'Estás seguro de enviar nuevamente la notificación a los pasajeros que no han confirmado su dirección?<br><br>Esta acción le notificará a cada pasajero por <b>WhatsApp, Correo y UP Mobile</b> para que confirme su dirección.',
          buttons: {
              confirm: {
                  text: 'Enviar',
                  btnClass: 'btn-success',
                  ...
                  action: function(){
                    ...
                    $.ajax({
                      ...
                      if(data.response==true){                       
                        ...
                        $.confirm({
                            title: 'Realizado!',
                            content: 'Fueron notificados todos los pasajeros!!!',
                            buttons: {
                                confirm: {
                                    text: 'Ok',
                                    btnClass: 'btn-success',
                        ...
                        $.alert({
                          title: 'Error!',
                          content: 'No se puede enviar nuevamente esta notificación, ya que fue enviada anteriormente.',
                        });
