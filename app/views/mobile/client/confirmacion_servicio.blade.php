        ...
        <table class="container" align="center" bgcolor="#FFFFFF" cellpadding="0" cellspacing="0" role="presentation" width="530">
          <tr>
            <td>
                    ...
                    <table cellpadding="0" cellspacing="0" role="presentation" width="100%">
                      <tr>
                        <td class="col" style="padding: 0 10px;">
                          <p style="color: #929292; text-align:center">Un servicio ha sido solicitado desde la aplicacion</p>
                          <div style="text-align: center">
                            <img class="align-sm-center" width="220" style="width: 220px; margin-bottom: 20px;" src="{{asset('img/mobile/logo_aotour_mobile_login.png')}}">
                          </div>
                        </td>
                      </tr>
                    </table>
              ...
              $us = DB::table('users')
              ->where('id',$servicio_aplicacion->user_id)
              ->pluck('centrodecosto_id');

              $client = DB::table('centrosdecosto')
              ->where('id',$us)
              ->pluck('razonsocial');
                    ...
                    <table cellpadding="0" cellspacing="0" role="presentation" width="100%">
                      <tr>
                        <td class="col" align="center" width="640">
                          <p style="text-align: left; color: white"><strong>Solicitud #: </strong>{{ucwords(strtolower($servicio_aplicacion->id))}}</p>
                          <p style="text-align: left; color: white"><strong>Cliente: </strong>{{ucwords(strtolower($client))}}</p>
                          <p style="text-align: left; color: white"><strong>Usuario:</strong> {{ucwords(strtolower($first_name))}} {{ucwords(strtolower($last_name))}}</p>
                          <table style="border: 1px solid white;" class="servicio">
                            <thead style="border: 1px solid white;">
                              <tr>
                                <th style="color: white; padding: 5px; width: 82px; text-align: center; border: 1px solid white;">Fecha<hr style="border: 1px dashed white;">Hora</th>
                                <th style="color: white; padding: 5px; text-align: center; border: 1px solid white; vertical-align: middle;">Recoger en</th>
                                <th style="color: white; padding: 5px; text-align: center; border: 1px solid white; vertical-align: middle;">Dejar En</th>
                              </tr>
                            </thead>
                            <tbody>
                              <tr>
                                <td style="color: white; padding: 5px; text-align: center; border: 1px solid white;">
                                  {{$servicio_aplicacion->fecha}} {{substr($servicio_aplicacion->hora, 0, 5)}}
                                </td>
                                <td style="color: white; padding: 5px; text-align: center; border: 1px solid white;">{{substr($servicio_aplicacion->address_recoger, 0, -8)}}</td>
                                <td style="color: white; padding: 5px; text-align: center; border: 1px solid white;">{{substr($servicio_aplicacion->address_destino, 0, -8)}}</td>
                              </tr>
                            </tbody>
                          </table>
                          @if ($servicio_aplicacion->nota!=null)
                            <div>
                              <p style="text-align: left; color: white; margin-bottom: 10px;"><strong>Detalles</strong></p>
                              <p style="color: white; text-align: justify; margin-bottom: 0; margin-top: 0px;">{{$servicio_aplicacion->nota}}</p>
                            </div>
                          @endif
                        </td>
                      </tr>
