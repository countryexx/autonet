    ...
    @if(isset($cuenta))
        <table class="table table-striped table-bordered hover" id="rutas_ejecutadas">
            ...
            @foreach($cuenta as $ruta)
                ...
                @endif
                <tr data-id="{{$ruta->servicio_id}}" @if($ruta->aceptado_liquidado!=null){{'class="success"'}}@elseif($ruta->aceptado_liquidado===null and $ruta->estado_correccion===0){{'class="danger"'}}@else{{'class="warning"'}}@endif >
                    ...
                    <td>
                        <input type="text" disabled="true" class="form-control input-font valor_liquidado" id="valor_liquidado" placeholder="Ingresar valor" value="{{$ruta->total_pagado}}" data-original="{{$ruta->total_pagado}}" data-id="{{$ruta->servicio_id}}">
                    </td>
                    <td>
                    	@if($ruta->valor_proveedor!=null)
                    		<?php $total = $total+$ruta->valor_proveedor; ?>
                            ...
                            @elseif($ruta->estado_correccion===0 and $ruta->aceptado_liquidado===null)
                                ...
                                <span class="last_value" >Corrección: <?php echo ' $ '.number_format($ruta->unitario_pagado_correccion)?></span>

                    		@elseif($ruta->estado_correccion==2 and $ruta->aceptado_liquidado==1)

                                <!-- CUANDO SE APROBÓ UN SERVICIO -->
                                <span class="last_value" >Valor Aprobado: <i class="fa fa-arrow-down" style="color: green" aria-hidden="true"></i></span>

                                <?php  $total2 = $total2+$ruta->unitario_pagado_correccion;?>

                            @elseif($ruta->estado_correccion==2 and $ruta->aceptado_liquidado===null)

                    			<span class="last_value" style="text-align: center; font-family: Bahnschrift; font-size: 13px">Corrección: <?php echo '$ '.number_format($ruta->unitario_pagado_correccion); ?></span>
                    			<br>
                    		...
                    		<input type="text" disabled="true" class="form-control input-font valor_cobrado1" id="valor_cobrado" placeholder="Ingresar valor" value="{{$ruta->valor_proveedor}}" data-original="{{$ruta->valor_proveedor}}" data-id="{{$ruta->servicio_id}}">
                    ...
                    <td>
                        ...
                        @if($ruta->aceptado_liquidado===1)
                            <span class="last_value" style="text-align: center; font-family: Bahnschrift; font-size: 13px">Liquidación: <?php echo '$ '.number_format($ruta->liquidado_original).' | '; ?></span>

                            <?php $dif = $ruta->liquidado_original -$ruta->valor_proveedor;
                            if($dif>0){
                                $color = 'blue';
                            }else if($dif<0){
                                $color = 'red';
                        ...
                        @else
                            ...
                            <?php $dif = $ruta->total_pagado-$ruta->valor_proveedor;
                                if($dif>0){
                                    $color = 'blue';
                                }else if($dif<0){
                                    $color = 'red';
                        ...
                        <span class="difference" style="color: <?php echo $color ?>">$ {{number_format($dif)}}</span>

                    </td>
                    <td style="text-align: center;">
                        @if($ruta->estado_correccion===null or $ruta->estado_correccion===0 and $estado_cuenta!=1)
                    ...
                    <td style="text-align: center;">
                        ...
                        @else
                            ...
                            <input type="checkbox" class="corregir" name="" data-id="{{$ruta->servicio_id}}" @if($estado_cuenta==3 or $estado_cuenta===1){{'disabled="true"'}}@endif>

                            <a id="ver_detalles" data-id="{{$ruta->servicio_id}}" data-cuenta="{{$id}}" nombre="{{$ruta->id}}" valor-servicio="{{$ruta->valor_proveedor}}" class="btn btn-list-table btn-danger back hidden"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>

                            <a style="margin-left: 2px" id="ver_detalles" data-id="{{$ruta->servicio_id}}" data-cuenta="{{$id}}" valor-liquidado="{{$ruta->total_pagado}}" class="btn btn-list-table btn-primary same hidden"><i class="fa fa-anchor" aria-hidden="true"></i></a>

                            <a style="margin-left: 2px" id="ver_detalles" data-id="{{$ruta->servicio_id}}" data-cuenta="{{$id}}" nombre="{{$ruta->id}}" class="btn btn-list-table btn-success save hidden"><i class="fa fa-check-square" aria-hidden="true"></i></a>
                        @endif

                    </td>
                    <td>
                        @if($ruta->conversacion!=null)
                            <a id="ver_motivo" option="1" data-id="{{$ruta->servicio_id}}" data-cuenta="{{$id}}" class="btn btn-dark ver_motivo"><i class="fa fa-comment" aria-hidden="true"></i></a>
                        @endif
                    </td>
                </tr>
				...
				<div class="row">
					<div class="panel panel-default" style="margin-top: 20px; margin-bottom: 0px">
					  ...
					  <div class="panel-body">
                            ...
                            @if($estado_cuenta===3)
                                    <a href="{{url('portalproveedores/pdfdownload/'.$id)}}" style="margin-bottom: 20px; margin-top: 20px; float: right;" class="btn btn-warning btn-icon input-font">Descargar Cuenta<i class="fa fa-file-pdf-o icon-btn"></i></a>
                            @else
                                    <a href="{{url('portalproveedores/pdfdownload/'.$id)}}" style="margin-bottom: 20px; margin-top: 20px; float: right;" class="btn btn-warning btn-icon input-font disabled">Descargar Cuenta<i class="fa fa-file-pdf-o icon-btn"></i></a>
                            @endif
