		...
		<div class="row">
			<table class="table tabla" style="margin-bottom:0px">
				<tbody>
					...
					<tr style="border-bottom: 1px solid #ddd;">
						<td>TOTAL COBRADO</td>
						<td><span style="color: #F47321; font-size: 16px">$ {{number_format($ordenes_facturacion->total_facturado_cliente)}}</span>
			...
			<tr>
				...
				<td>
					@if($servicio->consecutivo!=null)
						<span class="bolder" style="font-size: 10px; color: #F47321"><a href="../../facturacion/detallepagoproveedores/{{$consecutivo = str_replace('AP', '', $servicio->consecutivo)}}" title="Ver AP" target="_blank">
						#: {{$servicio->consecutivo}}</a></span><br>
				...
				@if($ordenes_facturacion->razonsocial==='COLEGIO ELISA BORRERO DE PASTRANA')
					<td>{{$servicio->valor_cobrado}}</td>
					<td>{{$servicio->valor_pagado}}</td>
					<td>{{$servicio->valor_cobrado}}</td>
					<td>{{$servicio->valor_pagado}}</td>
					<td>{{$servicio->utility}}</td>
				@else
						...
						<table style="margin-bottom: 10px" class="table table-bordered hover">
							<?php
								...
								if($ordenes_facturacion->razonsocial==='COLEGIO ELISA BORRERO DE PASTRANA'){
									...
									foreach ($servicios as $servicio){
										$total_generado_cobrado = $total_generado_cobrado + $servicio->valor_cobrado;
										$total_generado_pagado = $total_generado_pagado + $servicio->valor_pagado;
										$total_generado_utilidad = $total_generado_utilidad + $servicio->utility;
									}
