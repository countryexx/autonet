<html>
<head>
	<title>Autonet | Listado de Pagos a Auditar</title>
</head>
...
<body>
	...
	<div class="col-lg-12">
		<h3 class="h_titulo">PAGO A AUDITAR</h3>
	    	...
	    	<tbody>
	    		@foreach($pagos as $pago)
	    			<tr>
	    				...
	    				<td>
	    					<span style="line-height: 25px; margin-right:5px; font-size: 10px" class="text-info bolder">AUDITAR</span><div style="display:inline-block"><input id="{{$pago->id}}" type="checkbox" class="seguridad_social"></div>
	    				</td>
	    			</tr>
	    		@endforeach
	    	</tbody>
				...
				<div class="row">
					@if(isset($permisos->contabilidad->listado_de_pagos_auditar->auditar))
						@if($permisos->contabilidad->listado_de_pagos_auditar->auditar==='on')
				  		<button id="auditar_pagov2" style="margin-top: 15px; margin-bottom:10px" class="btn btn-default">AUDITAR</button>
						@else
...
<script type="text/javascript">
	...
	$('#auditar_pagov2').click(function(e){
                ...
                switch (index2){
                  case 10:
                    ...
                    if ($objeto.is(':checked')) {
                        idArray.push(id);
                        $(this).html('<span id="'+id+'" class="bolder seguridad_social" style="margin-right:4px; font-size: 10px">AUDITANDO...</span><i class="fa fa-spin fa-spinner"></i>');
                    }
                  break;
                }
            });
        ...
        if (idArray.length===0) {
            alert('No hay pagos por auditar!');
        }else{
            $.ajax({
                type: "post",
                url: "../auditarpago",
