...
<html>
  <head>
    ...
    <title>Autonet | Órdenes de Servicios BAQ</title>
  ...
  <body>
    ...
    <div class="col-lg-4"> <!-- FINALIZADOS -->
    ...
    <div class="col-lg-4"> <!-- ACTIVOS -->
      ...
      <div style="text-align: center; border: 1px solid; background: #C6E537; color: white;"><b>EN SERVICIO</b></div>
        ...
        <tbody>
          @foreach($servicios as $servicio)
            @if($servicio->estado_servicio_app===1)
              <tr id="{{$servicio->id}}" class="@if(intval($servicio->resaltar)===1){{'resaltar'}}@elseif($servicio->programado_app==1) {{'success'}} @endif">
                <td style="text-align: center; background: #C6E537; color: white; font-size: 15px;">
                  {{$servicio->razonsocial}} <br>
                  {{$servicio->nombre_completo}}<br>
                  {{$servicio->hora_servicio}}
                </td>
    ...
    <div class="col-lg-4"> <!-- PROGRAMADOS -->
      <div style="text-align: center; border: 1px solid; background: #00A0FC; color: white;"><b>PROGRAMADOS</b></div>
        ...
        <tbody>
          @foreach($servicios as $servicio)
            @if($servicio->estado_servicio_app===NULL)
              <tr id="{{$servicio->id}}" class="@if(intval($servicio->resaltar)===1){{'resaltar'}}@elseif($servicio->programado_app==1) {{'success'}} @endif">
                <td style="text-align: center; background: #00A0FC; color: white; font-size: 15px;">
                  {{$servicio->razonsocial}} <br>
                  {{$servicio->nombre_completo}}<br>
                  {{$servicio->hora_servicio}}
                </td>
    ...
    $( document ).ready(function() {
      ...
      var request = setInterval(function(){
        ...
        displayCount(count);
          ...
          $.ajax({
              type: 'post',
              url: url+'/transportesbaq/buscarordenes',
              processData: false,
              contentType: false,
              success: function(data) {

                  if(data.horaActual>='06:00' && data.horaActual<='08:00'){

                    //$('.doss').addClass('hidden');

                    //$('.hoy').addClass('hidden');
                    //$('.ayer').removeClass('hidden');

                    var ur = 'https://app.aotour.com.co/autonet/biblioteca_imagenes/control_ingreso/'+data.hoy+'.png';
                    //var ur = 'http://localhost/autonet/biblioteca_imagenes/control_ingreso/'+data.hoy+'.png'

                    var ur2 = 'https://app.aotour.com.co/autonet/biblioteca_imagenes/control_ingreso/'+data.ayer+'.png';
                    //var ur2 = 'http://localhost/autonet/biblioteca_imagenes/control_ingreso/'+data.ayer+'.png'

                    $('.hoy').attr('src',ur);
                    $('.ayer').attr('src',ur2);

                    if( $('.ayer').hasClass('hidden') ){

                      $('.ayer').removeClass('hidden');
                      $('.hoy').addClass('hidden');
                      console.log('ayer')

                    }else if( $('.hoy').hasClass('hidden') ){

                      $('.hoy').removeClass('hidden');
                      $('.ayer').addClass('hidden');
                      console.log('hoy')

                    }

                    console.log('test if '+data.horaActual)
                    ...
                    //console.log((ur));
                    $('.hoy').removeAttr('src').attr('src',ur);
                    //$('.ayer').attr('src',ur2);

                    $('.hoy').removeClass('hidden');
                    $('.ayer').addClass('hidden');
                  }
                  /*else if(data.horaActual>='10:46' && data.horaActual<='10:47'){

                    $('.doss').removeClass('hidden');
                    console.log('test else '+data.horaActual)

                  }else if(data.horaActual>='10:49' && data.horaActual<='10:50'){
                  ...
                  if(data.mensaje===true){
                    ...
                    $('#title').text('TIENES '+data.cantidad+' '+services+' PENDIENTE DE RECONFIRMACIÓN')
                    ...
                    for(i in data.servicios) {
                        ...
                        var btn = '<a href="tel:'+data.servicios[i].celular+'" type="button" class="btn btn-primary btn-block llamar" data-id="'+data.servicios[i].id+'">Llamar <i class="fa fa-phone" aria-hidden="true"></i></a>';

                        $('table#example4 tbody')
                        .append('<tr id="'+data.servicios[i].id+'">'+
                          '<td style="text-align: center; background: orange; color: white; font-size: 18px; font-family: Bahnschrift">'+data.servicios[i].razonsocial+' - '+data.servicios[i].nombre_completo+' - '+data.servicios[i].hora_servicio+'</td>'+
                          '<td>'+btn+'</td>'+
                          '</tr>');
        ...
        $table2.clear().draw();
          ...
          $.ajax({
              type: 'post',
              url: url+'/transportesbaq/buscarordenesactivas',
              processData: false,
              contentType: false,
              success: function(data) {
                  ...
                  if(data.mensaje===true){
                      ...
                      for(i in data.servicios) {
                        ...
                        ]).draw().nodes().to$().attr('style', 'text-align: center; background: #C6E537; color: white; font-size: 15px;');
          ...
          $table.clear().draw();
            ...
            $.ajax({
                type: 'post',
                url: url+'/transportesbaq/buscarordenesfinalizadas',
                processData: false,
                contentType: false,
                success: function(data) {
            ...
            $table3.clear().draw();
              ...
              $.ajax({
                  type: 'post',
                  url: url+'/transportesbaq/buscarordenesprogramadas',
                  processData: false,
                  contentType: false,
                  success: function(data) {
                      ...
                      if(data.mensaje===true){
                          ...
                          for(i in data.servicios) {
                            ...
                            if(data.servicios[i].aceptado_app==0 || data.servicios[i].aceptado_app==null){
                              var acept = '<a href="tel:'+data.servicios[i].celular+'" type="button" class="btn btn-danger btn-block no_aceptado" data-id="'+data.servicios[i].id+'"><i class="fa fa-phone" aria-hidden="true"></i></a>';
                              var btn = '';
                              var color = 'red';
                            }else{
                            ...
                            ]).draw().nodes().to$().attr('style', 'text-align: center; background: #00A0FC; color: white; font-size: 12px;');
          ...
          success: function(data) {
            if (data.response===true) {
              ...
              $('#example4 tbody tr[id="'+id+'"]').fadeOut(1000, function() {
                ...
                $(this).remove();
                $.alert('NO OLVIDES LLAMAR AL CONDUCTOR!');
