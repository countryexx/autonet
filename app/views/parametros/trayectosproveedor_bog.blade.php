<html>
<head>
	<title>Autonet | Trayectos Provedores Bogotá</title>
...
<h3 class="h_titulo">LISTADO DE TRAYECTOS - PROVEEDORES BOG</h3>
    ...
    @if(isset($ruta_general))
        ...
        <table id="exampless" class="table table-striped table-bordered hover" cellspacing="0" width="100%" style="margin-top: 70px">
            <thead>
                <tr>
                    <th>#</th>
                                                          
                    <th>Nombre</th>
                    <th style="text-align: center;">TARIFA AOTOUR
                        <!-- Small button groups (default and split) -->
                        
                        

                        <div class="btn-group" style="width: 100%">
                          <button style="float: right;" class="btn btn-success btn-xs dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i style="font-size: 12px" class="fa fa-caret-square-o-down" aria-hidden="true"></i>
                          </button>
                          <div class="dropdown-menu" style="width: 400px;">
                            ...
                            @endforeach
                                <li class="dropdown-item" type="button"><a href="{{url('tarifastraslados/trayectosaotourproveedorbog')}}">Ver Listado <i class="fa fa-share" aria-hidden="true"></i></a></li>
                          </div>
                        </div></th>
            ...
            @foreach($ruta_general as $ruta)
                <tr>
                    ...
                    <td style="text-align: center;">
                        ...
                        ->select('tarifas.id', 'tarifas.trayecto_id', 'tarifas.proveedor_auto', 'tarifas.proveedor_van', 'tarifas.centrodecosto_id', 'centrosdecosto.razonsocial', 'centrosdecosto.id as id_centro')
                        ...
                        @if($tarifa!=null)
                            <p class="bolder text-primary" style="margin: 0 !important; font-size: 12px;">$ {{number_format($tarifa->proveedor_auto)}} <a title="Actualizar Tarifa - Proveedor" id="" style="padding: 2px 3px; float: right;" data-id="{{$tarifa->id}}" data-trayecto="{{$tarifa->trayecto_id}}" data-nombre="{{$ruta->nombre}}" data-valor="{{$tarifa->proveedor_auto}}" data-valor2="{{$tarifa->proveedor_van}}" data-razonsocial="{{$tarifa->razonsocial}}" data-centro="{{$tarifa->id_centro}}" class="btn btn-warning editar_tarifas"><i class="fa fa-pencil-square-o"></i></a></p>
                        @else

                            N/A<a title="Añadir Tarifa - Proveedor" id="" style="padding: 2px 3px; float: right;" data-nombre="{{$ruta->nombre}}" data-trayecto="{{$ruta->id}}" data-centro="97" data-razonsocial="AUTO OCASIONAL TOUR" class="btn btn-info añadir_tarifas"><i class="fa fa-plus"></i></a>
                    ...
                    @foreach($query as $key)
                        ...
                        @if($tarifa!=null)
                            <p class="bolder text-primary" style="margin: 0 !important; font-size: 12px;">$ {{number_format($tarifa->proveedor_auto)}} <a title="Actualizar Tarifa" id="" style="padding: 2px 3px; float: right;" data-id="{{$tarifa->id}}" data-trayecto="{{$tarifa->trayecto_id}}" data-nombre="{{$ruta->nombre}}" data-valor="{{$tarifa->proveedor_auto}}" data-valor2="{{$tarifa->proveedor_van}}" data-razonsocial="{{$tarifa->razonsocial}}" data-centro="{{$tarifa->id_centro}}" class="btn btn-warning editar_tarifas"><i class="fa fa-pencil-square-o"></i></a></p>
                        @else
...
<div id="shortModal2" class="modal fade">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
                <h4 class="modal-title">Generar Aumento - Tarifa Proveedores</h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-xs-2">
...
<script type="text/javascript">
    ...
    $('#actualizar_tarifas').click(function(e) {
        ...
        if(sw==1){
            ...
            var valorv = $('#proveedor_van_editar').val();
            ...
            $.ajax({
              ...
              method: 'post',
              data: {id: id, valor: valor, valorv: valorv}
            }).done(function(data){
    ...
    $('#añadir_tarifas').click(function(e) {
        ...
        if(centro==97) {
            ...
            $.ajax({
              ...
              method: 'post',
              data: {id_trayecto: id_trayecto, centro: centro, valor: valor, localidad: 1}
            }).done(function(data){
    ...
    $('#aumentar_porcentaje').click(function(e){
        ...
        if(valor=='') {
            ...
            $.ajax({
              url: '../tarifastraslados/aumentarporcentajeproveedor',
              method: 'post',
    ...
    $('.editar_tarifas').click(function() {
        ...
        $('#proveedor_van_editar').val($(this).attr('data-valor2'))
