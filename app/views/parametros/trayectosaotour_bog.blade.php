<html>
<head>
	<title>Autonet | Traslados con Tarifa AOTOUR BOG</title>
...
<h3 class="h_titulo">LISTADO DE TRAYECTOS CON TARIFA AOTOUR - BOG </h3>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php 
                            ...
                            ->select('tarifas.id', 'tarifas.localidad', 'tarifas.trayecto_id', 'tarifas.cliente_auto', 'tarifas.centrodecosto_id', 'centrosdecosto.razonsocial', 'centrosdecosto.id as id_centro')
                        ...
                        @if($fee!=null)
                            <a title="Actualizar Tarifa" id="" style="padding: 2px 3px; float: right;" id-trayecto="{{$ruta->id}}" data-trayecto="{{$ruta->nombre}}" data-valor="{{$fee->cliente_auto}}" class="btn btn-primary paste_tarifas"><i class="fa fa-link"></i></a>
                        @endif
                    ...
                    @foreach($query as $key)
                        ...
                        @if($tarifa!=null)
                            <p class="bolder text-primary" style="margin: 0 !important; font-size: 10px;">$ {{number_format($tarifa->cliente_auto)}} <a title="Actualizar Tarifa" id="" style="padding: 2px 3px; float: right;" data-id="{{$tarifa->id}}" data-trayecto="{{$tarifa->trayecto_id}}" data-nombre="{{$ruta->nombre}}" data-valor="{{$tarifa->cliente_auto}}" data-razonsocial="{{$tarifa->razonsocial}}" data-centro="{{$tarifa->id_centro}}" class="btn btn-warning editar_tarifas"><i class="fa fa-pencil-square-o"></i></a></p>
                        @else
...
<div id="modal_paste" class="modal fade">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
                <h4 class="modal-title">Asignar Tarifa AOTOUR</h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-xs-6">
                        <label class="obligatorio" for="nombre_editar">Nombre de Trayecto</label>
                        <input class="form-control" disabled type="text" id="nombre_trayecto">
                    </div>
                    <div class="col-xs-12">
                        <fieldset style="margin-top: 10px; margin-bottom: 5px;"><legend class="title_tarifa">Tarifa</legend>
                            <div class="row">
                                <div class="col-xs-2">
                                    ...
                                    <input class="form-control" type="number" id="tarifa_auto">
                                </div>
                            </div>
                        </fieldset>
                    ...
                    <div class="col-lg-6 tabla_clientes">
                        <table name="clientes_fee" id="clientes_fee" class="table table-hover table-bordered tablesorter tabla" style="margin-top: 15px">
                             <thead>
                               <tr>
                                <td style="text-align: center;">Check All <br><center><input style="width: 15px; height: 15px;" class="select_al" type="checkbox" check="false"></center></td>
                                 <td style="text-align: center;">Nombre del Cliente</td>
                               </tr>
                             </thead>
                             <tbody>
            ...
            <div class="modal-footer">
                <button id="paste_tarifas" class="btn btn-success btn-icon">
                    ASIGNAR<i class="fa fa-floppy-o icon-btn"></i>
                </button>
                <a data-dismiss="modal" id="limpiar" class="btn btn-danger btn-icon">Cerrar<i class="fa fa-close icon-btn"></i></a>
            </div>
...
<script type="text/javascript">
    ...
    $('#paste_tarifas').click(function(e) {

        var id_trayecto = $(this).attr('id-trayecto')
        var valor_trayecto = $(this).attr('valor-trayecto')

        var idArray = [];

        e.preventDefault();
        $('#clientes_fee tbody tr').each(function(index){
        ...
        console.log(valor_trayecto);
        console.log(idArray);

        $.ajax({
          url: '../tarifastraslados/asignartao',
          method: 'post',
          data: {id_trayecto: id_trayecto, idArray: idArray, valor: valor_trayecto}
        }).done(function(data){

          if(data.respuesta==true){
            alert('¡Tarifa AOTOUR asignada!')
            location.reload();
          }else if(data.respuesta==false){
    ...
    $('#unificar_tarifa').click(function(e){
        ...
        $.ajax({
          url: '../tarifastraslados/unirclientebog',
          method: 'post',
          data: {idArray: idArray, localidad: 1}
        }).done(function(data){
    ...
    $('.paste_tarifas').click(function() {

        var nombre = $(this).attr('data-trayecto');
        var tarifa = $(this).attr('data-valor');
        var id = $(this).attr('id-trayecto');

        $('#nombre_trayecto').val(nombre);
        $('#tarifa_auto').val(tarifa);

        $('#paste_tarifas').attr('id-trayecto',id);
        $('#paste_tarifas').attr('valor-trayecto',tarifa);
    ...
    var $clientes_fee = $('#clientes_fee').DataTable({
        language: {
