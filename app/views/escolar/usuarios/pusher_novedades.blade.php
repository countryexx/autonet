...
<script>
  ...
  var channel = pusher.subscribe('escolar');

  channel.bind('rutas', function(data) {

    var info = data.info;
    var user = data.user;
    var novedad = data.novedad;
    //alert(user)
    //alert(info)
    ...
    if(novedad==0){
      $('#'+user+'').html('PASAJERO RECOGIDO');
    }else if(novedad==1){
      $('#'+user+'').html('NO SALIÓ A TIEMPO');
    }else if(novedad==2){
      $('#'+user+'').html('ESTUDIANTE INCAPACITADO');
    }else if(novedad==3){
      $('#'+user+'').html('LO RECOGIÓ EL ACUDIENTE');
    }
