...
<head>
    <meta charset="UTF-8">
    <title>Autonet | Nuevos Proveedores</title>
      ...
      <table id="examplenew" class="table table-bordered hover" cellspacing="0" width="100%">
          ...
          ?>
            <tr>
              ...
              <td><a title="CONDUCTOR: {{$proveedor->nombre_completo}} | VEHÍCULO: {{$proveedor->placa}}">{{$proveedor->razonsocial}} {{$proveedor->tipo_empresa}}</a></td>
              ...
              <td>
                ...
                @endif
                <!--<b>CONT</b> <i class="fa fa-{{$icon_contabilidad}}" aria-hidden="true" style="background-color: {{$fondo_contabilidad}}; color: white; padding: 5px"></i>   <i class="fa fa-arrows-h" aria-hidden="true"></i>
                <b>JUR</b> <i class="fa fa-{{$icon_juridica}}" aria-hidden="true" style="background-color: {{$fondo_juridica}}; color: white; padding: 5px"></i>-->
              </td>
              <td id="{{$proveedor->id}}">
                ...
                if($proveedor->autorizado_pc===1){ //ESTADO POLIZA CONTRACTUAL
                  $iconsv .= '<i class="fa fa-check" aria-hidden="true" style="font-size: 12px; color: yellow"></i>';
                  $sw3 = 1;
                }else if($proveedor->autorizado_pc===2){
                  $iconsv .= '<i class="fa fa-times" aria-hidden="true" style="font-size: 12px; color: red"></i>';
                  $sw2 = 1;
                }else{
                  $iconsv .= '<i class="fa fa-clock-o juridica_up" aria-hidden="true" style="font-size: 12px; color: white"></i>';
                ...
                if($proveedor->autorizado_pec===1){ //ESTADO POLIZA EXTRACONTRACTUAL
                  $iconsv .= '<i class="fa fa-check" aria-hidden="true" style="font-size: 12px; color: yellow"></i>';
                  $sw3 = 1;
                }else if($proveedor->autorizado_pec===2){
                  $iconsv .= '<i class="fa fa-times" aria-hidden="true" style="font-size: 12px; color: red"></i>';
                  $sw2 = 1;
                }else{
                  $iconsv .= '<i class="fa fa-clock-o juridica_up" aria-hidden="true" style="font-size: 12px; color: white"></i>';
              ...
              <td>
                ...
                @if( ($proveedor->estado_global===null and $proveedor->autorizacion_contabilidad===null) or ($proveedor->estado_global===null and $proveedor->autorizacion_contabilidad===1) or ($proveedor->estado_global===2 and $proveedor->autorizacion_contabilidad===1) )
                  ...
                  @if($proveedor->estado_global===3 and $proveedor->autorizacion_contabilidad===2)
                    <a title="Enviarlo a Capacitación" id="send_transportes" data-id="{{$proveedor->id}}" class="btn btn-success send_transportes disabled"><i class="fa fa-check" aria-hidden="true"></i></a>
                  @else
                    <a title="Solicitar Datos Financieros 11" id="send_financy" data-id="{{$proveedor->id}}" class="btn btn-primary send_financy disabled"><i class="fa fa-arrow-right" aria-hidden="true"></i> </a>
                  @endif
                ...
                @elseif($sw===0)
                  @if($sw2===0)
                    <!-- Enviar correcciones -->
                    <a title="ENVIAR DOCUMENTOS INCORRECTOS DE VUELTA AL PROVEEDOR" id="send_proveedor" data-id="{{$proveedor->id}}" class="btn btn-warning send_proveedor disabled"><i class="fa fa-reply-all" aria-hidden="true"></i></a>
                    @if( ($proveedor->estado_global===3 or $proveedor->estado_global===1) and $proveedor->autorizacion_contabilidad===1)
                      <a title="Enviarlo a Capacitación" id="send_cap " data-id="{{$proveedor->id}}" class="btn btn-primary send_cap"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                    @else
                      <!-- Aprobar datos Jurídicos -->
                      <a title="Enviarlo a Capacitación" id="send_financy" data-id="{{$proveedor->id}}" class="btn btn-primary disabled send_financy"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                    @endif
                  @else
                    <!-- Devolver documentos -->
                    <a title="ENVIAR DOCUMENTOS INCORRECTOS DE VUELTA AL PROVEEDOR" id="send_proveedor" data-id="{{$proveedor->id}}" class="btn btn-warning send_proveedor"><i class="fa fa-reply-all" aria-hidden="true"></i></a>
                    @if($proveedor->estado_global===3 and $proveedor->autorizacion_contabilidad===2)
                      <a title="Enviarlo a Capacitación" id="send_cap" data-id="{{$proveedor->id}}" class="btn btn-primary send_cap disabled"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>
                    @else
                      <a title="Solicitar Datos Financiesend_capos" id="send_financy" data-id="{{$proveedor->id}}" class="btn btn-primary send_financy disabled"><i class="fa fa-arrow-right" aria-hidden="true"></i> </a>
                    @endif
                  @endif
                @else
                  <a title="ENVIAR DOCUMENTOS INCORRECTOS DE VUELTA AL PROVEEDOR" id="send_proveedor" data-id="{{$proveedor->id}}" class="btn btn-warning send_proveedor disabled"><i class="fa fa-reply-all" aria-hidden="true"></i></a>
                  @if($proveedor->estado_global===3 and $proveedor->autorizacion_contabilidad===2)
                    <a title="Enviarlo a Capacitación" id="send_transportes" data-id="{{$proveedor->id}}" class="btn btn-success send_transportes disabled"><i class="fa fa-check" aria-hidden="true"></i></a>
                  @else
                    <a title="Solicitar Datos Financieros" id="send_financy" data-id="{{$proveedor->id}}" class="btn btn-primary send_financy disabled"><i class="fa fa-arrow-right" aria-hidden="true"></i> <i class="fa fa-money" aria-hidden="true"></i></a>
                  @endif
                @endif
                

                <!--@if($proveedor->estado_global===3 and $proveedor->autorizacion_contabilidad===2)
                  <a title="Enviarlo a Capacitación" id="send_transportes" data-id="{{$proveedor->id}}" class="btn btn-success send_transportes @if($proveedor->estado_global===1){{'disabled'}}@endif"><i class="fa fa-check" aria-hidden="true"></i></a>
                @else
                  <a title="Solicitar Datos Financieros" id="send_financy" data-id="{{$proveedor->id}}" class="btn btn-primary send_financy @if($proveedor->estado_global===1){{'disabled'}}@endif"><i class="fa fa-arrow-right" aria-hidden="true"></i> <i class="fa fa-money" aria-hidden="true"></i></a>
                @endif-->
              </td>
            </tr>
          @endforeach
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
          <fieldset style="margin-bottom: 5px;"><legend class="margin_label">Datos Generales</legend>
            <div class="row">
                <div class="col-xs-2">
                  ...
                  <select name="tipo_afiliado2" class="form-control input-font" id="tipo_afiliado2">
                    <option>-</option>
                    <option selected value="1">AFILIADO INTERNO</option>
            ...
            <center>
              <legend class="margin_label">Foto</legend>
              <img height="180px" width="200px" id="imagen">
            </center>
            <!--<div class="row">
              <div class="col-xs-3">
