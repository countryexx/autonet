...
<html>
  <head>
    ...
    <title>Autonet | Programación BAQ</title>
  ...
  <body>
    ...
    <div class="col-lg-12">
        <div class="row">
          <div class="col-lg-5">
            @include('servicios.menu_barranquilla')
          </div>
        </div>
        ...
        <form class="form-inline" id="form_buscar" action="{{url('reportes/excel')}}" method="post">
          ...
          <div class="col-lg-12" style="margin-bottom: 5px;">
              <div class="row">
                ...
                <button data-option="1" id="buscarbaq" class="btn btn-default btn-icon">Buscar<i class="fa fa-search icon-btn"></i></button>
        ...
        @if(isset($documentos))
          <table id="usuariosbaq" class="table table-bordered hover tabla" cellspacing="0" width="100%">
            <thead>
              <tr>
                <th></th>
                <th>ID SERVICIO</th>
                ...
                @foreach($documentos as $documento)
                    ...
                    <tr id="{{$documento->id}}" class="@if($o===1 and $sw===0){{'info'}}@elseif(intval($documento->servicio_id!=$conductor and $sw===0)){{'info'}}@elseif(intval($documento->servicio_id)===$conductor and $sw===0){{'info'}}@elseif(intval($documento->servicio_id)!=$conductor and $sw===1){{'success'}}@elseif(intval($documento->servicio_id)===$conductor and $sw===1){{'success'}}@endif">
                        ...
                        <td class="{{$documento->id}}">
                          ...
                          @elseif($documento->status===3 and $documento->status2===1)
                          ...
                          @elseif($documento->status===3 and $documento->status2===2)
                            <span style="font-size: 15px; color: orange">
                              <i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
                            </span>
                            <span style="font-size: 15px; color: red">
                          ...
                          @elseif($documento->status===3 and $documento->status2===null)
                            <span style="font-size: 15px; color: orange">
                              <i class="fa fa-exclamation-triangle" aria-hidden="true"></i>
                            </span>
                          @endif
