...
<html>
  <head>
    ...
    <title>Autonet | Control de Ingreso BOG</title>
  ...
  <body>
    ...
    <div class="col-lg-12">
        ...
        <h4 class="h_titulo" style="font-size: 16px">CONTROL DE INGRESO BOG, DÍA: <?php echo date('Y-m-d') ?></h4>
        @if(!$documentos)
          @if(isset($permisos->talentohumano->control_ingreso_bog->crear))
            @if($permisos->talentohumano->control_ingreso_bog->crear==='on')
              <button type="button" name="button" data-toggle="modal" class="btn btn-secundary aprobar_foto" id="generar_planilla" data-id="1" style="color: black" data-value="1">GENERAR PLANILLA DEL DÍA <i class="fa fa-bolt" aria-hidden="true"></i></button>
            @endif
          @endif
        @endif
            ...
            <tbody>
                @foreach($documentos as $documento)
                    ...
                    <tr id="{{$documento->id}}" class="">
                        ...
                        <td>
                          <div class="input-group">
                              <div class="input-group date" id="datetimepicker3">
                                ...
                                @if($documento->estado!=4 and Sentry::getUser()->id_empleado===$documento->dataid and date('H:i') >= '17:31' and $permisos->talentohumano->control_ingreso_bog->guardar_personal_bog==='on' and ($documento->hora_llegada!=null or $documento->hora_llegada!=''))
                        ...
                        @else
                          @if(Sentry::getUser()->id_empleado===$documento->dataid and date('H:i') >= '17:31' and $permisos->talentohumano->control_ingreso_bog->guardar_personal_bog==='on' and $documento->hora_llegada!=null and $documento->hora_llegada!='')
                          ...
                          @elseif(isset($permisos->talentohumano->control_ingreso_bog->crear))
                              @if($permisos->talentohumano->control_ingreso_bog->crear==='on')
